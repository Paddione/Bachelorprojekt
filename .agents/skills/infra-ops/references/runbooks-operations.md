...
> `kubernetes.pod-list-in-namespace({ namespace: "workspace" })` /
> `kubernetes.pod-log({ namespace: "workspace", name: "<pocket-id-pod>" })` (read-only);
...
> `kubernetes.pod-list-in-namespace({ namespace: "workspace" })` — alle Pods 1/1 Running?
> `kubernetes.pod-log({ namespace: "workspace", name: "<pocket-id|nextcloud|website>-pod" })`
