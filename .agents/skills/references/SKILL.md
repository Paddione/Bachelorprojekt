...
|-----------|-------------|
| **Claude Code** | Full — available as a listed skill; matches on description triggers |
| **opencode** | Full — available as a listed skill. All tools (CLI, MCP) are framework-agnostic |
| **agy** | Full — treats the canonical `.agents/skills` corpus as authoritative. All CLI tools and MCP calls work identically |
| **codex** | Full — discovers the canonical `.agents/skills` corpus |
