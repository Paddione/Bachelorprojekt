...
The shared [dev-flow-lifecycle](.agents/skills/references/dev-flow-lifecycle.md) owns transitions and roles; executable Git, verification and merge mechanics remain in their referenced SSOTs.
...
**Rule**: See `.agents/skills/lavish/SKILL.md#reload-safety` for the full lavish reload-safety protocol — never reload while a poll is outstanding, and check the last poll status before reloading.
