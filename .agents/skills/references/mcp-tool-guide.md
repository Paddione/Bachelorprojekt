...
> **`postgres.read-only-query` ist READ-ONLY und nimmt NUR `sql`.** Kein `connectionString`-Argument
...
- **Capability:** `postgres.read-only-query` (Param: **nur** `sql`)
- **Verfügbarkeits-Check — vor jedem Zugriff prüfen:** Der Server läuft als Port-Forward auf
  ...
  `postgres.read-only-query`-Aufruf die Erreichbarkeit bestätigen:
  ```bash
  curl -s --max-time 2 -o /dev/null -w '%{http_code}' \
    -X POST -H "Content-Type: application/json" -H "Accept: application/json, text/event-stream" \
    -d '{"jsonrpc":"2.0","id":1,"method":"initialize","params":{"protocolVersion":"2024-11-05","capabilities":{},"clientInfo":{"name":"hc","version":"1"}}}' \
  ...
  ```
  Schlägt der Check fehl oder die `postgres.read-only-query`-Fähigkeit ist im Harness nicht
  registriert → sofort auf den `psql()`-Fallback umschwenken, nicht auf den MCP-Weg beharren.
- **Brand-Bindung [T002278]:** dieser Server ist **brand-gebunden** an die **mentolder**-Datenbank.
  ...
  **stillschweigend die gleichnamige mentolder-Zeile** (das brand-gefilterte Query liefert leer
  ...
  `ticket.*` mit explizitem `brand`-Argument, **nicht** zu diesem Server.
...
- ⚠️ **Trotzdem nicht fuer Ticket-Zustand.** Der Grund ist jetzt ein anderer: `external_id`
  ...
  Status, `readiness`) IMMER `ticket.*` mit explizitem `brand` oder den
...
- **Wann bevorzugen:** Read-only SELECTs gegen `knowledge.*`, `v_timeline` oder andere
  Nicht-Ticket-Tabellen. Für Ticket-Queries → `ticket.get` /
  `ticket.list` mit gesetztem `brand`.
...
- **Tools (Auswahl):** `kubernetes.pod-list-in-namespace`, `pods_list`, `pods_log`,
...
- **Fallback (Daemon down):** Status/Queue → `postgres.read-only-query`/`psql` auf
