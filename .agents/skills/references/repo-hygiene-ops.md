...
* **Ticket schließen, sobald `mergedAt` gesetzt ist** (nur wenn `$TICKET_ID` gefunden;
  ...
  > `ticket.transition-status({ id: "$TICKET_ID", status: "done", resolution: "<fixed|shipped>" })`
  > `ticket.add-comment({ id: "$TICKET_ID", body: "PR #<number> merged." })`
...
Für Ticketstatus ist der kanonische Weg ohnehin `ticket.get` /
`ticket.list`, nicht ein geratenes CLI-Subkommando (siehe
[`mcp-tool-guide.md`](mcp-tool-guide.md)).
...
1. **Title-Dedupe-Guard [T001210]:** Vor dem Anlegen nach einem offenen Ticket mit gleichem
   ...
   > ```
   > ticket.list({ … })      # offene Tickets, wie gehabt
   > mishap.get-buffer({})    # ungeflushte Befunde
   > ```
...
# Factory-Queue
factory.status({})   # liefert queue_depth + is_running
```
...
| 3 | `Factory queue_depth > 3` | **Factory-Health check**: `factory.ask({ question: "Sind alle Worker gesund? Gibt es blockierte Jobs?" })`. Ggf. `factory.trigger({})`. |
