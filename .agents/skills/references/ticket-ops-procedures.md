...
**DB-Zugriff:** Reads MCP-first via `postgres.read-only-query`; der `psql()`-Fallback-Helper
...
**MCP-first** (`mcp-postgres`, read-only): pass the query below to `postgres.read-only-query({ sql: "…" })`. Fallback: `psql -c "<query>"` via the `psql()` helper above.
...
2. **`child_of`-Links setzen** (für jedes Kind-Ticket):
   ```
   ticket.link({ from: "<child_ext_id>", to: "<BATCH_PARENT_ID>", kind: "child_of" })
   ```
...
> `ticket.set-readiness-flag({ id: "T000XXX", flag: "spec_skizziert", value: true })`
> `ticket.set-plan-meta({ id: "T000XXX", effort: "mittel", depends_on: "T000YYY" })`
> `ticket.add-comment({ id: "T000XXX", body: "## Klärungsrunde …" })`
