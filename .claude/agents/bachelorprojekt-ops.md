...
- The old `mentolder` and `korczewski` kubeconfig contexts are DEAD — use `fleet` for all kubectl commands. The one remaining non-fleet context is `k3d-mentolder-dev` (dev stack on local Proxmox VM 10.0.0.26; `k3s-1` was decommissioned 2026-05-31 due to memory corruption).
