...
**Baseline-Stichtag:** `2026-07-01` · **Zuletzt gemessen:** `2026-08-18` · **Dashboard:** Homepage-Section `#health`
...
| **G-SEC03** | SealedSecret-Rotation | 2 Tage ✓ | ≤ 90 Tage | `git log -1 --format='%at' -- environments/sealed-secrets/*.yaml \| ...` |
| **G-SEC04** | Sealing-Cert Restlaufzeit | 3600 Tage ✓ | ≥ 30 Tage | `openssl x509 -enddate -noout -in environments/certs/*.pem` |
...
| **G-SPEC02** | Changes >30 Tage | 4 ⚠ | 0 | `for d in openspec/changes/*/; do ... done` |
...
| **G-DOC02** | Root-CLAUDE.md Zeilen | 278 ⚠ | ≤ 200 | Positiv-Anker: `CLAUDE.md` fehlt ⇒ n/a; `wc -l < CLAUDE.md` |
...
| **G-AGENTIC11** | CLAUDE.md opencode-Liste vs opencode.jsonc | 2 ⚠ | 0 | `comm -3 <(grep opencode-Liste \| extract backtick-names) <(mcp_servers opencode.jsonc)` |
| **G-AGENTIC12** | .mcp.json-Server undokumentiert | 1 ⚠ | 0 | `for s in $(mcp_servers .mcp.json); grep -q -- "$s" mcp-tool-guide.md || count` |
...
| **G-DB04** | Backup-Alter (h) seit letztem db-backup-Job | 578 ⚠ | ≤ 26h | `db_scalar Backup-Alter (health-goals-check.sh); Regressionswache T001738` |
...
| **G-OPS02** | Container-Restarts <24h (fleet, beide Brands) | 6 ⚠ | ≤ 3 | `kubectl get pods -o json` + Python-Filter `lastState.terminated.finishedAt` < 24h (health-goals-check.sh) |
| **G-OPS03** | Live-TLS-Cert-Restlaufzeit (Tage, min beider Brands) | 10 ⚠ | ≥ 14 | `echo \| openssl s_client -servername web.<brand>.de -connect …:443 \| openssl x509 -enddate -noout` (health-goals-check.sh, mit Retry gegen Multi-A-Record-Transienten) |
