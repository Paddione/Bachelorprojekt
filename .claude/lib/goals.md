...
| **G-DOC02** | Root-CLAUDE.md Zeilen | 203 ⚠ | ≤ 200 | `wc -l < CLAUDE.md` |
...
| **G-DB04** | Backup-Alter (h) seit letztem db-backup-Job | 52 ⚠ | ≤ 26h | `db_scalar Backup-Alter (health-goals-check.sh); Regressionswache T001738` |
...
| **G-OPS02** | Container-Restarts <24h (fleet, beide Brands) | 13 ⚠ | ≤ 3 | `kubectl get pods -o json` + Python-Filter `lastState.terminated.finishedAt` < 24h (health-goals-check.sh) |
| **G-OPS03** | Live-TLS-Cert-Restlaufzeit (Tage, min beider Brands) | 32 ✓ | ≥ 14 | `echo \| openssl s_client -servername web.<brand>.de -connect …:443 \| openssl x509 -enddate -noout` (health-goals-check.sh, mit Retry gegen Multi-A-Record-Transienten) |
