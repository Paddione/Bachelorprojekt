...
**Baseline-Stichtag:** `2026-07-01` · **Zuletzt gemessen:** `2026-08-13` · **Dashboard:** Homepage-Section `#health`
...
| **G-SEC03** | SealedSecret-Rotation | 17 Tage ✓ | ≤ 90 Tage | `git log -1 --format='%at' -- environments/sealed-secrets/*.yaml \| ...` |
| **G-SEC04** | Sealing-Cert Restlaufzeit | 3605 Tage ✓ | ≥ 30 Tage | `openssl x509 -enddate -noout -in environments/certs/*.pem` |
...
| **G-DB04** | Backup-Alter (h) seit letztem db-backup-Job | 459 ⚠ | ≤ 26h | `db_scalar Backup-Alter (health-goals-check.sh); Regressionswache T001738` |
...
| **G-OPS03** | Live-TLS-Cert-Restlaufzeit (Tage, min beider Brands) | 15 ✓ | ≥ 14 | `echo \| openssl s_client -servername web.<brand>.de -connect …:443 \| openssl x509 -enddate -noout` (health-goals-check.sh, mit Retry gegen Multi-A-Record-Transienten) |
