...
2. Spawne über das `Agent`/`Task`-Tool einen Subagenten, **provisioniert gemäß** [subagent-provisioning](file:///home/patrick/Bachelorprojekt/.claude/skills/references/subagent-provisioning.md) (Modell · Effort · Kontext):
   ...
   - **Kontext-Injektion** (er hat sonst KEINEN Kontext — gib ihm alles explizit):
     ...
     - **Plan-Lint strukturelle Anforderungen:** Der Plan muss unbedingt die vom Linter (`scripts/plan-lint.sh`) geforderten formalen Strukturen einhalten:
        1. **YAML-Frontmatter:** Muss am Dateianfang vorhanden sein und die Schlüssel `title`, `ticket_id`, `domains` und `status` definieren.
        2. **Header-Titel:** Muss mit `# [Name] Implementation Plan` (oder ähnlich) beginnen.
        3. **Dateistruktur-Sektion:** Muss eine `## File Structure` Sektion enthalten, welche die vorgeschlagenen Dateiänderungen listet.
        4. **Failing-Test-Schritt (für Fix-Pläne):** Muss mindestens einen Test-Schritt enthalten, der fehlschlagen soll und explizit die Phrase `expected: FAIL` (oder ähnlich lautende Phrasen wie `verify it fails`) enthält, um die Reproduktion der Regression zu belegen.
        5. **Keine unquoted Platzhalter:** Im Fließtext dürfen keine unquoted Platzhalter wie `TBD`, `TODO`, `FIXME` oder `???` stehen (falls diese referenziert werden müssen, müssen sie in Backticks wie `` `TODO` `` eingeschlossen sein).
     - **Auftrag:** „Lies die Spec UND `.claude/skills/references/plan-quality-gates.md`. Rufe `superpowers:writing-plans` auf und schreibe den Implementierungsplan **ausschließlich** nach `openspec/changes/<slug>/tasks.md` (OpenSpec-Format: H2-Operationsheader im Delta, H3-Requirement, H4-Scenario im `specs/<capability>.md`). Der Plan MUSS die plan-lint Strukturvorgaben erfüllen: YAML-Frontmatter mit `title`/`ticket_id`/`domains`/`status`, Header `# Implementation Plan`, eine Sektion `## File Structure`, bei Fix-Plänen einen fehlschlagenden Test-Step mit der Phrase `expected: FAIL`, sowie keinerlei unquoted Platzhalter (`TBD`, `TODO`, `FIXME`, `???`). Der finale Verifikations-Task des Plans MUSS `task test:changed`, `task freshness:regenerate` und `task freshness:check` als Steps enthalten (CI-Äquivalent inkl. S1–S4-Ratchet); nach Test-Änderungen zusätzlich `task test:inventory` + Commit des Inventars. Vor dem Commit: `task test:openspec` (oder `bash scripts/openspec.sh validate`) — muss grün sein. Starte KEINE Implementierung (nur Plan schreiben, dann STOPP). Gib den Plan-Pfad (`openspec/changes/<slug>/tasks.md`) und eine 3-Zeilen-Zusammenfassung zurück."
