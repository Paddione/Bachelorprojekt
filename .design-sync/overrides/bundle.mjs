...
// svgr stub: intercept ?react SVG imports (vite-plugin-svgr) → stub React component
// so ServiceRow/icons.ts renders without the ?react query that esbuild can't resolve.
const svgrPlugin = {
  name: 'svgr-stub',
  setup(b) {
    b.onResolve({ filter: /\.svg\?react$/ }, (args) => ({
      path: args.path,
      namespace: 'svgr-stub',
    }));
    b.onLoad({ filter: /.*/, namespace: 'svgr-stub' }, () => ({
      contents: `import React from 'react';
export default function SvgIcon(props) {
  return React.createElement('span', { 'aria-hidden': 'true', style: { display: 'inline-block', width: '1em', height: '1em' }, ...props });
}`,
      loader: 'js',
    }));
  },
...
function sharedBuildOptions({ nodePaths, tsconfig }) {
  ...
  const plugins = [svgrPlugin, reactShim];
