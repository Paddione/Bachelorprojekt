...
Phasen-Telemetrie (PFLICHT für verify — Schritt 6 erzwingt `verify:done` fail-closed): [phase-events](references/phase-events.md).
...
Nach grünen Tests das `verify done`-Event setzen — Aufrufe (MCP-first + Bash-Fallback): [phase-events](references/phase-events.md).
