...
Rollen- und Übergabevertrag: [dev-flow-lifecycle](.agents/skills/references/dev-flow-lifecycle.md). Diese Skill enthält nur die Execute-Gates, die Mechanik steht in den verlinkten Referenzen. Interaktive Ausführung über die OpenCode-Slash-Commands `dev-flow-exe.md` und `dev-flow-execute.md`.
...
Befehlsfolgen: [dev-flow-execute-phases](.agents/skills/references/dev-flow-execute-phases.md). Hier nur Leistung und Abbruchbedingung:
...
> `ticket-mcp-node_record_phase_event({ id: "$TICKET_ID", phase: "implement", state: "entered", driver: "devflow", detail: "Subagent gestartet · agent_id=$IMPLEMENTER_AGENT_ID" })`
Fallback:
```bash
./scripts/ticket.sh phase "$TICKET_ID" implement entered --driver devflow --detail "Subagent gestartet" || true
```

Delegiere die **gesamte Implementierung an EINEN frischen Subagenten**. Du behältst den Plan-Kontext und verifizierst danach unabhängig. Kein Per-Task-Fan-out über `superpowers:subagent-driven-development`: diese Skill läuft oft schon als delegierte Ebene, verschachtelte Delegation sprengt den Kontext (162k-Prompt-Lehre, [subagent-provisioning](.agents/skills/references/subagent-provisioning.md)). Der Implementer ruft `superpowers:executing-plans` **in-context** auf. Fan-out nur als bewusste Eskalation, wenn viele unabhängige Tasks den Einzel-Implementer ans Kontext-Limit bringen.
...
> `ticket-mcp-node_record_phase_event({ id: "$TICKET_ID", phase: "implement", state: "done", driver: "devflow", detail: "Implementierung fertig" })`
> `ticket-mcp-node_record_phase_event({ id: "$TICKET_ID", phase: "verify", state: "entered", driver: "devflow", detail: "task test:changed + freshness" })`
Verifikation: die vier Befehle + `./tests/runner.sh local <FA-XX oder SA-XX>` bei
Manifest-Änderungen. **SSOT:** [verification-block](.agents/skills/references/verification-block.md).
...
> `ticket-mcp-node_record_phase_event({ id: "$TICKET_ID", phase: "verify", state: "done", driver: "devflow", detail: "Tests grün · freshness OK" })`
...
```
TypeScript/pnpm-Gotchas in Worktrees: [dev-flow-gotchas](.agents/skills/references/dev-flow-gotchas.md).
...
- **Merge-Wait-Loop zuerst (T001149-M1):** [ci-fix-loop](.agents/skills/references/ci-fix-loop.md)
...
**Orchestrator-Schritt, nie als Hintergrund-Monitor** [T001969 Mishap 1, T002351-M3]. Auto-Merge ist seit 3.8 angefordert und greift bei grünen Required Checks. Details/Required-Check-Liste: [ci-fix-loop](.agents/skills/references/ci-fix-loop.md).
```bash
...
> check (T000722). Required-Check-Liste: [ci-fix-loop](.agents/skills/references/ci-fix-loop.md).
...
Merge abwarten ([ci-fix-loop](.agents/skills/references/ci-fix-loop.md) §"PR-Merge-Wait-Loop",
...
```
**Deploy-Mapping (Single Source of Truth):** [deploy-routing](.agents/skills/references/deploy-routing.md) — nur dort pflegen.
...
> `.agents/skills/references/grilling-to-ticket.md`.
