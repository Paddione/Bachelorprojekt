...
1. Run the bundled discovery script to get a Kubernetes resource inventory:
   ```bash
   .opencode/skills/gitops-repo-audit/scripts/discover.sh -d <repo-root>
   ```
...
.opencode/skills/gitops-repo-audit/scripts/validate.sh -d <repo-root> ${bundle:+-b "$bundle"}
```
...
1. Run the bundled check script:
   ```
   .opencode/skills/gitops-repo-audit/scripts/check-deprecated.sh -d <repo-root>
   ```
