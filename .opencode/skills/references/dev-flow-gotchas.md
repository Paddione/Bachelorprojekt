...
The shared [dev-flow-lifecycle](.opencode/skills/references/dev-flow-lifecycle.md) owns transitions and roles; executable Git, verification and merge mechanics remain in their referenced SSOTs.
...
**Rule**: See `.opencode/skills/lavish/SKILL.md#reload-safety` for the full lavish reload-safety protocol — never reload while a poll is outstanding, and check the last poll status before reloading.
