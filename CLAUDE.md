...
- **BATS CRLF tolerance**: When writing guards (`grep -qE`) against `.ps1` files, account for CRLF line endings by using `[[:space:]]*$` instead of `$` (the `\r` character belongs to the POSIX `[[:space:]` class). This prevents false-negative test failures on Windows-origin scripts.
