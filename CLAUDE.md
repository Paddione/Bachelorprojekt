...
- **Website, Brett, Docs, Videovault, and Mediaviewer-Widget images use `:latest` intentionally** (`k3d/website.yaml`, `k3d/brett.yaml`, `k3d/docs.yaml`, `k3d/videovault.yaml`, `k3d/mediaviewer-widget.yaml`). CI warns about `:latest` for all five; do not "fix" these tags to a digest — each image is rebuilt and re-imported/pushed on every release (`task feature:brett`, `task docs:deploy`, `task feature:website`).
...
- **Generated artifacts are conflict magnets — resolve with `git checkout --ours`.** `docs/generated/**`, `docs/code-quality/repo-index.json`, and `k3d/docs-content-built/architecture/index.html` are auto-regenerated by `freshness-regen.yml` after every main push. Any PR that also committed a freshness regen will conflict. Resolution: `git checkout --ours <file>` for each of these files during rebase, then `git add` them. The `.gitattributes` `merge=ours` driver automates this when `task secrets:install-hooks` has been run (registers `git config merge.ours.driver true`).
