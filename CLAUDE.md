...
- **mentolder**: Still a STANDALONE k3s cluster, ALIVE, running on Hetzner host nodes (`gekko-hetzner-*`) plus one home-LAN worker (`k3s-1`, Proxmox VM on pvemain at 10.0.3.1, wg-mesh 192.168.100.20). The RPi workers (`k3w-1/2/3`) are decommissioned. `k3s-2` and `k3s-3` have been repurposed as `devc-2`/`devc-3` (HA dev cluster servers at 10.0.0.22/23, not on wg-mesh). Its migration onto the fleet is a *reversible DNS flip* (see `docs/fleet-stage2-cutover-runbook.md`) and is NOT done yet.
