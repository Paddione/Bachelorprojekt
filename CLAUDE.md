...
**Deliverable-Check vor manuellem `done`/`shipped` (M10, T002506):** Bei manuellen Closures (kein Auto-Merge — z. B. Epics, die über mehrere PRs laufen) VOR dem Setzen auf `done`/`shipped` prüfen, dass alle im Plan deklarierten Deliverable-Dateien tatsächlich auf `origin/main` existieren (`git ls-tree -r --name-only origin/main | grep -qxF <pfad>` — plattformrobust, funktioniert auch unter Git Bash/MSYS). Ein `done` ohne Deliverable ist Prozess-Drift und erzwingt einen nachtraeglichen Pflaster-Commit (beobachtet bei T002459/P5.5). Redaktioonerlicher Hinweis, kein automatisierter Guard.
...
Prerequisites: Docker, kubectl, `task` (go-task). `k3d` wird nicht mehr gebraucht — kein Pfad im Repo ruft es noch auf (T900120).
...
bash scripts/vda.sh oracle 'validate the kubernetes manifests before committing'
```
...
- **`fleet`**: The unified cluster — **3 CP nodes** (`pk-hetzner-4/6/8`, control-plane + etcd) plus Worker aus dem `gekko-hetzner-*`-Pool. **Die konkrete Worker-Liste steht bewusst nicht mehr in dieser Datei**, aus demselben Grund wie die Pod- und Replica-Zahlen: sie veraltet schneller, als sie gepflegt wird. Zweimal belegt — T900038 (die Zusicherung „both brands at 26/26 pods“ behauptete über sechs Wochen einen Zustand, den es nicht mehr gab) und T900247 (die Zeile nannte „2 worker nodes (gekko-hetzner-3/4)“ und ausdrücklich, `gekko-hetzner-2` sei **nicht** im Cluster; die Messung am 2026-09-20 fand sechs Ready-Knoten, gekko-hetzner-2 eingeschlossen). Belastbar ist nur die Messung: `kubectl --context fleet get nodes -o wide` für die Knoten, `kubectl --context fleet get deploy -n workspace` bzw. `-n workspace-korczewski` für die Workloads. Laufende Workloads trägt derzeit **nur** `workspace` (mentolder); `workspace-korczewski` ist eingefroren (siehe korczewski-Zeile oben). Single source of truth for all production workloads — `fleet` is the only live **prod** context. Daneben ist `devmesh` aktiv (Entwicklung, ADR-008); einen Context `hetzner` gibt es nicht. Jeder weitere von `kubectl config get-contexts` gelistete Context (`k3d-*`, `devc`, `gekko-hetzner-2-dev`, …) zeigt auf abgebaute Hardware — gegen den lebenden Cluster prüfen, nicht gegen diese Zeile.
