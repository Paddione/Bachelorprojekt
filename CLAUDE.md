...
### Cluster Topology & Nodes (Fleet Stage 2 — DNS CUTOVER COMPLETE 2026-05-30)
- **mentolder (BRAND)**: DNS for `mentolder.de` now routes to the **`fleet`** cluster (pk-hetzner-4/6/8 IPs: 204.168.244.104/37.27.251.38/62.238.23.79). The mentolder-standalone k3s cluster on `gekko-hetzner-*` nodes is still running but **no longer serving live traffic** — it is a warm fallback until decommission (Phase 3). Operate the mentolder brand via the **`fleet`** context, namespace `workspace` (`ENV=fleet-mentolder`). The old `mentolder` context still reaches the standalone cluster (for rollback/decommission). `k3s-1` (10.0.3.1, wg-mesh 192.168.100.20) remains in the standalone cluster AND hosts the dev k3d stack.
- **korczewski (BRAND)**: The standalone korczewski cluster has been **TORN DOWN** (intentional). Its hosts `pk-hetzner-4/6/8` now run the unified **`fleet`** k3s cluster. DNS for `korczewski.de` routes to fleet. Operate the korczewski brand via the **`fleet`** context, namespace `workspace-korczewski` (`ENV=fleet-korczewski`). The old `korczewski` kubeconfig context (`204.168.244.104:6443`) is **DEAD** — that IP now serves the fleet k3s CA (x509 error, T000340).
- **`fleet`**: The unified cluster on `pk-hetzner-4/6/8`. **Both brands LIVE on fleet** — namespace `workspace` (mentolder brand) and `workspace-korczewski` each at **26/26** pods; `workspace:verify` passes (11/12 checks) for both. Wildcard certs READY (T000351 resolved). Collabora at `office.mentolder.de` + `office.korczewski.de` (PR #1222). Website, CoTURN, Collabora all deployed. **Remaining (Phase 3):** decommission mentolder-standalone (scale down gekko-hetzner-* nodes), remove standalone kubeconfig context, reclaim hardware.
...
Other workflows: `e2e.yml` (nightly Playwright against mentolder standalone and korczewski brand on fleet), `build-brett.yml` (tag `brett-v*`), `build-docs.yml` (tag `docs-v*`), `build-collabora.yml`, `build-transcriber.yml`, `build-website.yml` / `build-website-korczewski.yml` (auto build+rollout on `website/**` push to main), `dev-auto-deploy.yml` (auto-deploy to dev.mentolder.de on relevant push), `dev-smoke.yml` (nightly BATS against dev.mentolder.de at 05:00 UTC).
