...
- **`fleet`**: The unified cluster on `pk-hetzner-4/6/8`. Currently has ONLY cert-manager + Traefik; **zero brand workloads** (namespaces `workspace` and `workspace-korczewski` are empty). `task fleet:deploy` has NOT been run yet, so `korczewski.de` returns HTTP 404 behind the Traefik default cert — down-but-EXPECTED until fleet:deploy. The cutover mechanism is merged (PR #1189): `scripts/fleet-dns-cutover.sh`, `task fleet:dns:cutover|rollback`; the LIVE cutover is NOT done.
