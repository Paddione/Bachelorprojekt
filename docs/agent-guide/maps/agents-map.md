...
| local | subagent | llamacpp-local/Qwen3.8-27B-gsq | nein | Sole local implementation subagent: Qwen3.8-27B dense via llama.cpp :1919 (RTX 5070 Ti, GSQ-RCO IQ3_XXS-mtp, 153600 served KV, ~96 tok/s decode, gemessen 2026-09-24) [T900348, T900359]. Sequential dispatch with per-packet budget_tokens (S ~32k / M ~80k / L ~100k). |
...
| qwen38-primary | primary | llamacpp-local/Qwen3.8-27B-gsq | ja | PRIMARY: Local Qwen3.8-27B dense via llama.cpp (:1919, RTX 5070 Ti, served KV 153600) (llamacpp-local/Qwen3.8-27B-gsq) [T900348]. Autonomous ticket-hammering agent: prioritizes critical tickets, plans (/dev-flow-plan), explores (/opsx:explore), executes, and archives. |
| reviewer | subagent | llamacpp-local/Qwen3.8-27B-gsq | nein | Factory reviewer role — read/grep/glob/tests only; no edit/write/bash/task dispatch (Orchestrator applies review edits) [T900074] |
