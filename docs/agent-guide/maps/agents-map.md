...
| local | subagent | llamacpp-local/Qwen3.8-27B-dualgpu | nein | Sole local implementation subagent: Qwen3.8-27B dense via llama.cpp :1919 (dual-GPU 5070 Ti + 3060 Ti, 153600 served KV, ~35 tok/s decode, gemessen 2026-09-23) [T900348]. Sequential dispatch with per-packet budget_tokens (S ~32k / M ~80k / L ~120k). |
...
| qwen38-primary | primary | llamacpp-local/Qwen3.8-27B-dualgpu | ja | PRIMARY: Local Qwen3.8-27B dense via llama.cpp (:1919, dual-GPU, served KV 153600) (llamacpp-local/Qwen3.8-27B-dualgpu) [T900348]. Autonomous ticket-hammering agent: prioritizes critical tickets, plans (/dev-flow-plan), explores (/opsx:explore), executes, and archives. |
| reviewer | subagent | llamacpp-local/Qwen3.8-27B-dualgpu | nein | Factory reviewer role — read/grep/glob/tests only; no edit/write/bash/task dispatch (Orchestrator applies review edits) [T900074] |
