...
| orchestrator | primary | opencode-zen/muse-spark-1.3-contributor-free | ja | Primary orchestrator on Muse Spark 1.3 Contributor Free (OpenCode Zen, free tier, 1M ctx, 131k output) [T900210]. Also drives the factory executor (opencode-exec.sh runs --agent orchestrator). Dispatches the single local subagent (budget_tokens per packet) + cloud escalation. Eskalationskette: local (2x) → planner-muse (Go, M2) → deepseek-helper-go → deepseek-helper → deepseek-pro |
...
| planner-muse | primary | opencode-go/muse-spark-1.3-contributor | ja | PLANNER fallback on Muse Spark 1.3 Contributor via OpenCode Go (subscription rail, 1M ctx, 131k output) [T900210]. First cloud escalation after local stalls (M2); dispatched by orchestrator, big-pickle, qwen38-primary. Free-first chain: orchestrator (Zen-free) → planner-muse (Go) → deepseek rails. |
