---
title: Korczewski Mustervertrag + Musterrechnung Seed — Implementation Plan
ticket_id: T000192
domains: []
status: active
pr_number: null
...
# Korczewski Mustervertrag + Musterrechnung Seed — Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Seed a Mustervertrag (`document_templates`) and Musterrechnung invoice-text defaults (`site_settings`) into the korczewski cluster DB, and verify the admin content-hub Rechnungen tab works.

**Architecture:** Pure database seeding — no website code changes. One idempotent SQL file applied via `kubectl exec … psql`. Verification queries confirm the data is present after the seed.

**Tech Stack:** PostgreSQL 16, `kubectl exec` against `shared-db-*` pod in `workspace-korczewski` namespace, korczewski context.

---

### Task 1: Write the seed SQL file

**Files:**
- Create: `scripts/one-shot/20260524-korczewski-muster-seed.sql`

- [ ] **Step 1: Create the seed SQL file**

The file seeds:
1. One `document_templates` row (Mustervertrag) using the korczewski contract starter HTML
2. Five `site_settings` rows (Musterrechnung invoice-text defaults) scoped to brand `korczewski`

Both inserts are idempotent (safe to re-run).

```sql
-- scripts/one-shot/20260524-korczewski-muster-seed.sql
-- Korczewski: seed Mustervertrag + Musterrechnung invoice-text defaults
-- Safe to re-run: document_templates uses DO NOTHING on title match,
-- site_settings uses DO UPDATE SET value=EXCLUDED.value so admin edits win
-- on the next run but the row always exists.

BEGIN;

-- ── Mustervertrag ────────────────────────────────────────────────────────────
-- document_templates has no brand column; all templates are cluster-wide.
-- Title 'Mustervertrag' uniqueness is enforced by the DO NOTHING guard.
...
WHERE NOT EXISTS (SELECT 1 FROM document_templates WHERE title = 'Mustervertrag');

-- ── Musterrechnung (invoice-text defaults for korczewski brand) ──────────────
-- site_settings PK is (brand, key). korczewski is confirmed in public.brands.
-- DO UPDATE ensures re-runs preserve admin edits only if value changed externally;
-- for initial seeding, INSERT where not exists is sufficient.
INSERT INTO site_settings (brand, key, value) VALUES
  ('korczewski', 'invoice_intro_text',              'für folgende Leistungen stelle ich Ihnen in Rechnung:'),
  ('korczewski', 'invoice_kleinunternehmer_notice', 'Kein Ausweis der Umsatzsteuer aufgrund der Anwendung der Kleinunternehmerregelung gemäß § 19 UStG.'),
  ('korczewski', 'invoice_outro_text',              'Vielen Dank für Ihr Vertrauen!'),
  ('korczewski', 'invoice_email_subject',           'Rechnung {{number}}'),
  ('korczewski', 'invoice_email_body',              E'Sehr geehrte/r {{customer_name}},\n\nanbei erhalten Sie Rechnung {{number}} über {{gross_amount}} €.\n\nBitte überweisen Sie den Betrag bis {{due_date}} unter Angabe des Verwendungszwecks „{{payment_reference}}".\n\nMit freundlichen Grüßen\n{{seller_name}}')
...
```bash
cd /tmp/wt-korczewski-muster
git add scripts/one-shot/20260524-korczewski-muster-seed.sql
git commit -m "chore(db): add korczewski Mustervertrag + Musterrechnung seed SQL [T000192]"
```

---

### Task 2: Apply seed to korczewski DB
...
Expected output: `Pod: pod/shared-db-XXXXXXXX-XXXXX`
...
Expected output: both counts are `0` (pre-seed state).
...
If `INSERT 0 0` appears instead of `INSERT 0 1` for the Mustervertrag, a row with title `'Mustervertrag'` already exists — the `WHERE NOT EXISTS` guard fired correctly. Not an error.
...
Expected output:
- `document_templates`: 1 row, title `Mustervertrag`, `body_chars` ≥ 12000
- `site_settings`: 5 rows for brand `korczewski`, one per invoice key
...
```bash
cd /tmp/wt-korczewski-muster
git push -u origin feature/korczewski-muster
```
...
```bash
gh pr create \
  --title "feat(db): seed Mustervertrag + Musterrechnung for korczewski [T000192]" \
  --body "$(cat <<'EOF'
## Summary
- Seeds `Mustervertrag` into `document_templates` on the korczewski cluster using the existing `contract-korczewski.html` starter
- Seeds five `invoice_*` default text values into `site_settings` for brand `korczewski` (Musterrechnung)
- Admin `/admin/inhalte?tab=rechnungen` already rendered correctly via hardcoded defaults; persistent values are now stored in DB

## Test plan
- Applied seed SQL directly against korczewski `shared-db` via `kubectl exec`
- Verified `document_templates` has 1 row (Mustervertrag, ≥12000 chars HTML body)
- Verified `site_settings` has 5 rows for brand=korczewski with invoice_ keys
- Rechnungen tab at `/admin/inhalte?tab=rechnungen` confirmed accessible (no CSS/auth/routing blocks found by code analysis)
...
No deployment needed — this is a DB seed, not a code/image change.
