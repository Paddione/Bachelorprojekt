...
`dev-flow-batch` Modus 2 mit echter KI-basierter Zerlegung und file-isolierten Sub-Features ausstatten. `pipeline.js` auf ≤600 Zeilen bringen durch Extraktion der Decompose-Logik in eine neue Datei `pipeline-decompose.js`.

## S1-Budget-Analyse

| Datei | Aktuell | Limit | Status |
|-------|---------|-------|--------|
| `scripts/factory/pipeline.js` | 777 Zeilen | 600 | Über Limit — Extraktion in pipeline-decompose.js reduziert auf <600 |
| `scripts/factory/pipeline-decompose.js` | neu | 600 | Neue Datei, bleibt <600 |
...
### T2 — `scripts/factory/pipeline-decompose.js` erstellen (NEU, <600 Zeilen)
...
- [ ] `node --check scripts/factory/pipeline-decompose.js` muss sauber durchlaufen

### T3 — `scripts/factory/pipeline.js` anpassen (<600 Zeilen nach Extraktion)

- [ ] Decompose-Logik (aktueller `plan:decompose`-Agent-Block) in pipeline-decompose.js verlagert — pipeline.js referenziert `decomposeFeature` nicht direkt (Workflow-Import-Verbot), stattdessen: pipeline-decompose.js wird als separates Workflow-Skript via `workflow()` aufgerufen ODER die Funktion wird inline in pipeline.js belassen und die Extraktion betrifft nur Hilfsfunktionen
  - **Konkrete Strategie**: Hilfsfunktionen `provision()`, `chooseModel()`, `chooseEffort()`, `buildContextHints()`, `routeProviderSync()`, `releaseSlotSync()`, `routerSource()`, `routerTier()` (~80 Zeilen) in pipeline-decompose.js als eigenständige Sammlung auslagern; pipeline.js `require()`-t diese via CommonJS wenn im Workflow-Kontext erlaubt — falls nicht: Funktionen bleiben inline, aber redundante Kommentare + veraltete Spike-Kommentare werden bereinigt bis <600 Zeilen
...
- [ ] Modus 2 Schritt 2 "Decompose-Subagent": Verweis auf `pipeline-decompose.js` + Erklärung des `apiBalance`-Parameters
...
- [ ] `node --check scripts/factory/pipeline.js` und `node --check scripts/factory/pipeline-decompose.js`
...
- **Workflow-Import-Verbot**: CommonJS `require()` ist in Workflow-Skripten erlaubt, ESM `import` nicht. pipeline-decompose.js muss ohne Top-Level-Import auskommen.
