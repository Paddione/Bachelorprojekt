...
- [x] `apps/_schema.json` erstellen. Definiert das JSON-Schema für `app.yaml` (Felder: `name`, `title`, `description`, `kustomize`, `domains`, `oidc`, `secrets`, `requires`, `resources`).
- [x] Ordner `apps/whiteboard/` anlegen und `apps/whiteboard/app.yaml` basierend auf dem bestehenden Whiteboard-Dienst schreiben.
- [x] BATS-Test oder ein Node-Skript zur Validierung des Schemas hinzufügen.
...
- [x] `scripts/app-install.sh` anlegen. Implementiert:
...
- [x] Registrieren des Installers in `Taskfile.yml` unter `app:install` und `app:validate`.
- **Acceptance:** `task app:install -- whiteboard --dry-run` zeigt alle geplanten Mutationen.

### Task 3 — Astro & Svelte UI im Cockpit
- [x] Astro-Seite `website/src/pages/admin/app-catalog.astro` anlegen (geschützt analog `/admin/planungsbuero`).
- [x] Svelte-Komponente `website/src/components/admin/AppCatalog.svelte` anlegen, welche alle Manifeste aus `apps/` scannt, deren Status aus `apps/installed-<env>.json` liest und das Grid anzeigt.
- [x] Der Install-Button zeigt in v1 eine Anleitung mit dem `task app:install`-Befehl für den Administrator (kein direktes Schreiben aus der UI in v1).
...
- [x] BATS-Tests für `scripts/app-install.sh` in `tests/local/FA-SF-57-app-catalog.bats` hinzufügen, um Fehlerfälle (ungültiges Manifest, fehlende Abhängigkeiten, dry-run Modifikationen) abzudecken.
- [x] Playwright E2E-Test `tests/e2e/specs/app-catalog.spec.ts` erstellen, um die UI-Darstellung und Berechtigungsprüfungen abzusichern.
