<script lang="ts">
  ...
  const types = [
    ...
    { value: 'kennenlernen', label: 'Kennenlerngesprach (45 Min / 20 EUR)' },
  ...
  async function handleSubmit(e: Event) {
    ...
    try {
      ...
      if (response.ok) {
        result = { success: true, message: 'Vielen Dank! Ihre Nachricht wurde gesendet. Ich melde mich in Kurze bei Ihnen.' };
        name = '';
        email = '';
        phone = '';
        type = 'allgemein';
