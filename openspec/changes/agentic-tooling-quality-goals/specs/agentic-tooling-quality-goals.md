...
Diese Capability definiert quantifizierbare, reproduzierbar gemessene Qualitätsziele für die
Agentic-Tooling-Artefakte des Repos selbst — Custom-Subagents (`.claude/agents/*.md`), Skills
(`.claude/skills/*/SKILL.md`), MCP-Server-Konfiguration (`.mcp.json` / `.opencode/opencode.jsonc`)
und agentische Slash-Commands (`.claude/commands/`, `.opencode/commands/`). Jedes Requirement
entspricht einem Ziel `G-AGENTIC01`–`G-AGENTIC17` in `.claude/lib/goals.md`, gemessen über
