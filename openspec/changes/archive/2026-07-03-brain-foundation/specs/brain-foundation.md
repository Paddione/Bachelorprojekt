...
Der `brain-foundation`-Spec definiert das Fundament des gemeinsamen LLM-gepflegten
Wissens-Repos (`Paddione/brain`) nach Karpathys LLM-Wiki-Pattern — Change 1/7 des Epics
**brain-llm-wiki** (PRD an Ticket T001566). Er legt fest, welche Seed-Struktur das brain-Repo
trägt, welche Schema-Konventionen (Frontmatter-Pflichtfelder, Wikilinks, `source::`-Rückverweise)
gelten, und welche Qualitäts-Gates (toter-Wikilink-Check, Frontmatter-Lint, Secret-Scan) die CI
des brain-Repos durchsetzt. Zusätzlich verankert er die Anforderungen an den idempotenten Bootstrap,
der aus dem Bachelorprojekt heraus test- und wiederholbar seedet. Nicht-Ziele (Quartz-Deploy,
Ingest, Merge-Hook, MCP-Server) sind Folge-Changes (Ticket T001567) und ausdrücklich außerhalb
dieses Fundaments.
