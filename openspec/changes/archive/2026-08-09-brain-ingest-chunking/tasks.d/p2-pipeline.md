...
- [x] Nach dem Pilot-Zuschnitt (`:89-94`) und **vor** der Slug-Inventur eine
...
- [x] `CHUNK_TOTAL="$(wc -l < "$CHUNKS_TSV")"` ermitteln und zusammen mit
...
- [x] `:96-99` von der Worklist auf `$CHUNKS_TSV` umstellen: `awk -F'\t'
...
- [x] Ergebnis wie bisher über `jq -R . | jq -s .` nach `$SLUGS_JSON`
  schreiben und die bestehende Zeile `Slug inventory: …` beibehalten.
- [x] Prüfen, dass `$SLUGS_JSON` vor dem ersten `process_page`-Aufruf
...
- [x] Signatur auf `process_page <src_path> <chunk_file> <chunk_slug> <index>`
...
- [x] Hash-Vergleich (`:153-155`) auf den Chunk beziehen:
  `sha256sum "$chunk_file"`, State-Schlüssel `"$src_path#$index"`. Ein
  unveränderter Chunk liefert weiterhin Rückgabewert 2 (skip).
- [x] Transform-Aufruf (`:174`) mit `$chunk_file` und `$chunk_slug` ausführen;
  Frontmatter-Prüfung (`:179-182`) unverändert lassen.
- [x] Den Transform-Aufruf mit `BRAIN_SOURCE_PATH="$src_path"` in der Umgebung
...
- [x] State-Schreibblock (`:186-192`) auf den Chunk-Schlüssel umstellen und um
...
- [x] Direkt nach dem Chunk-Vorpass alle State-Schlüssel ermitteln, deren
...
- [x] Für jeden solchen Eintrag die zugehörige `wiki/<slug>.md` löschen und den
...
- [x] Begründung im Kommentar festhalten: `brain-ingest-prune.sh` erkennt diese
...
- [x] `while IFS=$'\t' read -r …` (`:205`) auf `$CHUNKS_TSV` und dessen fünf
  Spalten umstellen; die MOC-Zeile (Chunk-Slug gleich Quell-Slug bzw. Index 0)
  wird übersprungen, da sie ohne LLM erzeugt wird.
- [x] Fortschrittsausgabe (`:223`) auf `$CHUNK_TOTAL` beziehen und den
  Chunk-Slug statt des Quellpfads anzeigen.
- [x] Ergebniszeile je Job (`:221`) von reinem Exit-Code auf
...
- [x] Auswertungsschleife (`:228-235`) auf die neue Zeilenform anpassen und
...
- [x] Die Meldungen um `:239` und `:267-287` sprachlich von „Quellen" auf
  „Chunks" umstellen; die Schwellenlogik (`INGEST_MAX_FAIL_ABS`,
  `INGEST_MAX_FAIL_PCT`, `INGEST_MIN_SAMPLE`, `:262-274`) bleibt unverändert.
- [x] Im Kommentarblock `:241-261` ergänzen, dass sich die absolute Schwelle
...
- [x] Am Anfang von Phase 2b (`:290-296`) über die MOC-Zeilen aus
  `$CHUNKS_TSV` laufen und jede MOC-Datei nach `$BRAIN_REPO/wiki/<slug>.md`
  kopieren — ohne LLM-Aufruf (D4).
- [x] Beim Kopieren jede Listenzeile der Form `- [[<chunk-slug>]]` verwerfen,
...
- [x] Je kopierter MOC einen State-Eintrag `"$src_path#moc"` mit `slug`
  ...
  behandeln.
- [x] Anzahl kopierter Eltern-MOCs ausgeben (`Parent MOCs: N`).

### Aufgabe 8 — `brain-ingest-coverage.sh`: Gerüst und Argumente

`scripts/brain-ingest-coverage.sh`

- [x] Neues Skript mit `#!/usr/bin/env bash`, `set -euo pipefail`, Kopfkommentar
  (Zweck, Usage, Env) im Stil von `scripts/brain-ingest-prune.sh:1-7`.
- [x] Argumente: `--worklist <tsv>` (Pflicht), `--root <dir>` (Vorgabe:
...
- [x] `BRAIN_MIN_COVERAGE_PCT` als Env-Vorgabe mit Wert 95;
  `--min-pct` überschreibt sie.
- [x] Fehlende Pflichtdateien mit klarer Meldung und Exit 1 abweisen.

### Aufgabe 9 — `brain-ingest-coverage.sh`: Messung und fail-closed Abbruch

`scripts/brain-ingest-coverage.sh`

- [x] Bezugsmenge bilden: alle Quellpfade, die in `--delivered` mit mindestens
...
- [x] Nenner: Summe von `wc -c` über die Quelldateien dieser Menge, aufgelöst
  über `$ROOT/<src_path>` anhand Spalte 1 der Worklist.
- [x] Zähler: Summe der Zeichenspalte aus `--delivered` über alle Zeilen mit
...
- [x] Prozentwert ganzzahlig als `zaehler * 100 / nenner` berechnen; ein Nenner
  von 0 (keine versuchte Quelle) ist kein Fehler und gibt
  `Coverage: keine versuchte Quelle — Gate übersprungen` mit Exit 0 aus.
- [x] Erfolgsausgabe: `Coverage: <pct>% (Schwelle <min>%, <zaehler> von
  <nenner> Zeichen, <k> versuchte Quellen)`.
- [x] Unterhalb der Schwelle nach stderr dieselbe Zeile plus den Hinweis auf
...
- [x] In Phase 3 (`:424-428`) **vor** `cd "$BRAIN_REPO"` und damit vor
...
- [x] Exit ≠ 0 beendet den Lauf mit Exit 1, bevor Phase 4 (`:476-488`) etwas
  ausliefert — die Reihenfolge ist der ganze Zweck des Gates (REQ-k4-07).
- [x] Erfolgsfall wie die anderen Gates protokollieren
  (`  Coverage gate: PASS`).
...
- [x] Beide `trap`-Zeilen (`:83`, `:203`) um `$CHUNKS_TSV`, `$DELIVERED_TSV`
...
- [x] Dry-Run-Zusammenfassung (`:479-484`) und Schlussausgabe (`:538-540`) um
  die Chunk-Zahl erweitern: `Chunks: <CHUNK_TOTAL> aus <TOTAL> Quellen`.
- [x] PR-Text in `:519-531` auf Chunk-Zählung anpassen und das Coverage-Gate in
...
- [x] Nach Abschluss von Aufgabe 11 messen:
  `wc -l scripts/brain-ingest.sh`.
- [x] Liegt das Ergebnis über 640 Zeilen (80 % der wirksamen Schwelle 800), die
...
- [x] Coverage-Skript zusätzlich einzeln gegen die im Lauf entstandenen
...
- [x] `bash -n scripts/brain-ingest.sh scripts/brain-ingest-coverage.sh` sowie
  `shellcheck` auf beide Dateien laufen lassen.
- [x] Eine Wiki-Chunk-Seite öffnen und prüfen, dass ihre `source::`-Zeile den
