...
- [x] **`lib/sdlc/leitstand-kpi.ts` anlegen (pure Aggregation).** Funktionen
      ...
      duplizieren).
- [x] **`KpiGrid.svelte` anlegen.** Leerlauf-Modul der Z4: fetcht `/sdlc/api/delivery-metrics`
      ...
      Platzhalterzahlen.
- [x] **`Kontextzone.svelte` umbauen.** Leerlauf-Zweig (keine Selektion) rendert `KpiGrid`
      statt des bisherigen Platzhalter-/Minimal-Zustands; Stations- und Ticket-Zweige bleiben
      unverändert.
- [x] **`DeckPlattform.svelte` auf echte Quellen umstellen.** Vorhandene
...
- [x] **Verwaiste Dateien löschen.** `components/sdlc/factory/KostenTab.svelte` (einziger
...
- [x] **`observability.astro` löschen + Redirect.** Seite entfernen; in
...
- [x] **purpose-Registry pflegen.** In `lib/sdlc/leitstand-purpose-registry.ts` Einträge für
