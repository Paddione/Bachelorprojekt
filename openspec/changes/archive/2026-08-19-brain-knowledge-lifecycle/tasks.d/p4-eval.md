...
- [x] Eine ausschließlich standardbibliotheksbasierte CLI
...
- [x] Den Evalset-Vertrag als UTF-8-JSONL mit genau einem Objekt pro nichtleerer Zeile
...
- [x] Das Modul `scripts/brain-index.py` relativ zu `__file__` mit
...
- [x] Determinismus absichern: Evalzeilen in Dateireihenfolge verarbeiten, keine Uhrzeit in
...
- [x] Für jeden Fall die geordneten Ergebnis-Slugs aus den höchstens `k` Treffern ableiten.
...
- [x] JSON-Ausgabe mit stabilem Schema erzeugen:
...
- [x] Human-Ausgabe aus denselben berechneten Daten rendern: eine stabile Kopfzeile mit
...
- [x] Baseline-only Semantik umsetzen: Ein gültig ausgeführter Lauf endet unabhängig von den
...
- [x] In `taskfiles/Taskfile.brain.yaml` diese vier Tasks ergänzen; alle Argumente werden über
...
- [x] Die Taskfile ändert weder `scripts/brain-lifecycle-audit.py` aus p1 noch
...
- [x] Nach Integration von p5 zuerst dessen fokussierten Test gegen den noch fehlenden Runner
      ausführen:
...
- [x] Nach Implementierung Syntax und Task-Erreichbarkeit ohne Netz oder Mutation prüfen:
...
- [x] Den p5-Test grün ausführen. Er muss denselben Fixture-Wiki- und JSONL-Input zweimal mit
