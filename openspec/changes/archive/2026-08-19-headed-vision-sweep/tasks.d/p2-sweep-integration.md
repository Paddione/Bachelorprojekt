...
- [x] **Typ und Pfad in den Helfern ergänzen** (`visual-sweep-helpers.ts`): ein Typ
...
- [x] **Aufrufpunkt setzen** (`visual-sweep.spec.ts`): unmittelbar nachdem der Screenshot
...
- [x] **Screenshot als JPEG aufnehmen, wenn die Stufe an ist.** `type: 'jpeg', quality: 80`
...
- [x] **`VISION_MAX_ROUTES` beachten.** Ist die Variable gesetzt, werden nur die ersten N Routen
...
- [x] **Ergebnisdatei schreiben**, wenn der Sweep durchgelaufen ist — getrennt von
...
- [x] **Den Read-only-Guard gegenprüfen.** `installReadOnlyGuard` bricht Nicht-GET/HEAD-Anfragen
...
- [x] **Kein Einfluss auf das Ergebnis des Laufs.** Der Sweep darf durch ein negatives oder
