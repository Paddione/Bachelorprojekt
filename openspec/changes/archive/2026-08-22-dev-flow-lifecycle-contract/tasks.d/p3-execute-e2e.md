...
- [x] Replace Execute's non-linear numbered narrative with explicit Implementer, Orchestrator
...
- [x] Place independent review and fail-closed `assert-phase-chain` before the auto-merge
  request in both the skill and phase reference.
- [x] Define the Orchestrator exception loop through confirmed `MERGED`: route later red checks,
...
- [x] Keep the fresh Finalizer waiting concurrently and require it to call only the idempotent
  finalize script after merge confirmation. It must not close on timeout, open/closed PR state,
  late CI failure or conflict.
- [x] Remove repeated Freshness, auto-merge, finalizer and incident prose from Execute while
  retaining the literal anchors required by existing safety guards and reference links.
- [x] Reframe E2E as a specialized test-only Chore: preserve live discovery, Mentolder-only
