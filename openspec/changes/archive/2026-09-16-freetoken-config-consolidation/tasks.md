...
- [x] `.opencode/opencode.jsonc`: `"model": "freetoken-local/active"` →
  `"model": "llamacpp-local/qwen38-220k"`.
- [x] Begleitkommentar (Zeilen 3-8) korrigieren: nicht mehr "der alte llama.cpp-Stack (:18235)
...
- [x] `scripts/llm/loadouts.json`: `factory.model` von `"freetoken-local"` auf `"qwen38-220k"`.
- [x] Den `freetoken-local`-Loadout-Eintrag (slug `freetoken-local`, Port 1919, `managed:
  external`) vollstaendig entfernen.
- [x] `task llm:loadouts:check` (falls vorhanden) bzw. `jq empty scripts/llm/loadouts.json`
  gegen valides JSON pruefen.

### Task 4 — Routing-Skripte von :1919 befreien
- [x] `scripts/factory/route-provider.sh`: `FT_LOCAL_PROVIDER="freetoken"` →
...
- [x] `scripts/llm/routing-check.sh`: `http://127.0.0.1:1919` aus der `for url in ...`-Probe-Liste
  entfernen; Kommentar (Zeilen 27-30) korrigieren. Fail-Soft-Verhalten (Exit 0 ohne erreichbares
  Backend) unveraendert lassen.
- [x] `scripts/llm/measure-factory-context.mjs`: Kommentarzeile 11 (Verweis auf geloeschtes
...
- [x] `docs/agent-guide/registry/agents.yaml`: alle 11 `model: freetoken-local/active*`-Eintraege
...
- [x] `openspec/specs/llm-local-dev.md`: MODIFIED-Delta aus
...
- [x] `openspec/changes/freetoken-local-backend/` archivieren (Feature vollstaendig entfernt,
...
### Task 7 — Verifikation
- [x] `tests/unit/lib/bats-core/bin/bats tests/spec/local-llm-proxy/factory-default-not-freetoken.bats`
