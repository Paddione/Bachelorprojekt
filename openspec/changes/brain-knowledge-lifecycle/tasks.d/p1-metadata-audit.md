...
- [ ] Vor Implementierung die vom Tests-Partial bereitgestellten fokussierten Fälle ausführen:
...
- [ ] Die Tests müssen mindestens diese beobachtbaren Verträge prüfen:
...
- [ ] Neue ausführbare Standardbibliothek-CLI mit `argparse` anlegen. Pflichtargumente sind
...
- [ ] Pure Helfer mit konkreten Signaturen implementieren:
...
- [ ] `apply_metadata()` arbeitet zeilenbasiert nur im ersten `---`-Block, entfernt dort
...
- [ ] Der Filter berechnet `source_revision = sha256_file(Path(args.source))`, übernimmt nie
...
- [ ] Neben den bestehenden Script-Konstanten `METADATA_SCRIPT="$HERE/brain-page-metadata.py"`
  definieren und mit den vorhandenen `[ -f ... ]`-Preflights fail-closed prüfen.
- [ ] Einmal pro Lauf nach der Argumentvalidierung
...
- [ ] `source_kind_for_group()` neben der realen Funktion `determine_group()` ergänzen und die
...
- [ ] In der realen Funktion `process_page()` `src_file="$REPO_ROOT/$src_path"` verwenden und
...
- [ ] Die bestehende State-Idempotenz bleibt unverändert chunk-basiert. Bei unverändertem Chunk
...
- [ ] Neue ausführbare Standardbibliothek-CLI anlegen:
...
- [ ] Pure Datenstruktur `PageRecord` als `@dataclass(frozen=True)` und Helfer mit Signaturen
...
- [ ] Lifecycle-Felder mit derselben ISO-Logik wie der Metadaten-Filter normalisieren. Intervalle
...
- [ ] `superseded_by` gegen die Menge vorhandener Slugs prüfen. Ein unbekanntes Ziel erzeugt
  `missing_superseded_target` mit `slug` und `target`.
- [ ] Den vorhandenen Body-Anker `source:: Bachelorprojekt <relative-path>` strikt parsen. Nur
  ...
  interpretiert.
- [ ] Claims ausschließlich aus vollständigen Body-Zeilen der Form
...
- [ ] Findings deterministisch nach `(code, slug, other_slug, claim_key)` sortieren. JSON ist ein
...
- [ ] Unter „Frontmatter-Pflichtfelder“ ergänzen: neu kompilierte Seiten tragen die flachen
...
- [ ] Kontrollierte `source_kind`-Werte, SHA-256-Revision der ursprünglichen Quelle, ISO-8601-
...
- [ ] Unter einer kurzen Sektion „Claims und Lifecycle-Audit“ die exakte Syntax
...
- [ ] Den fokussierten Vertrag erneut ausführen:
...
- [ ] S4 und Report-only-Verhalten zusätzlich prüfen: `rg -n 'brain-page-metadata.py|brain-lifecycle-audit.py' scripts/brain-ingest.sh tests/spec/brain-knowledge-lifecycle.bats`
