...
- [ ] Standardbibliothek-basiertes Python-CLI mit Subcommands `fetch`, `stage`,
...
- [ ] State-Root ist per `--state-dir` testbar, sonst
...
- [ ] `fetch` darf ausschließlich diese vier repo-/PR-gebundenen Aufrufe über
...
- [ ] Dokumentierte Bedienfolge implementieren:
...
- [ ] Aus den GitHub-Antworten ausschließlich folgende Felder übernehmen:
...
- [ ] Vor jedem Persistieren Secret- und PII-Redaction auf alle Freitextfelder
      ...
      umgangen.
- [ ] Das persistierte JSON erhält `schema_version: 1`, `repository`, `pull_request`,
...
- [ ] Temporärdateien ausschließlich mit `tempfile` im Zielverzeichnis und Modus
...
- [ ] `stage` erzeugt aus der redigierten Evidenz deterministisch ein Markdown-
...
- [ ] Nur Aussagen mit einer konkreten Evidence-ID/URL übernehmen. Freie
...
- [ ] Vor dem Schreiben denselben Redactor erneut auf das gerenderte Markdown
...
- [ ] `approve` verlangt einen gültigen `--slug` (`^[a-z0-9]+(?:-[a-z0-9]+)*$`) und
...
- [ ] Vor Freigabe Kandidat erneut parsen und prüfen: Repo/PR/SHA entsprechen den
...
- [ ] Zieldatei ist deterministisch
      ...
      duplizieren.
- [ ] Freigabe ist append-only: existiert der Zielpfad, nur byte-identischen Inhalt
...
- [ ] Das Skript muss per Kopfkommentar und Policy referenziert sein (S4). Danach
...
- [ ] Unter `groups:` exakt diese Gruppe ergänzen:

```yaml
  github-reviewed: docs/brain-expertise/approved/*.md
```

- [ ] Unter `type_map.defaults` `github-reviewed: note` und unter `tag_defaults`
...
- [ ] Mit einem temporären Repo-Root prüfen, dass ein Dokument unter
...
- [ ] Deutschsprachige Policy mit Zweck, erlaubtem Scope und Bedienfolge anlegen.
...
- [ ] Explizite Verbote dokumentieren: keine organisationsweite/ambiente Suche, keine
...
- [ ] Review-Checkliste aufnehmen: Scope und SHA stimmen; Evidence-Links sind
...
- [ ] Die Policy ist selbst ein bewusst freigegebenes Dokument in der
