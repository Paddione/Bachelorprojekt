...
- [x] Vor Änderungen an den Testdateien die von den Implementierungs-Partials benannten
...
- [x] `tests/spec/brain-foundation/knowledge-lifecycle.bats` mit Fixture-Buildern für ein
...
- [x] Feste ISO-8601-Werte verwenden und die halboffene Grenze
...
- [x] Zwei Seiten mit demselben vollständigen `claim:: <key> = <value>`-Key, verschiedenen
...
- [x] Vor jedem Audit SHA-256-Inventare aller Wiki-Dateien erfassen und sie nach Text- sowie
...
- [x] Im selben Foundation-Bundle ein temporäres `gh`-Stub vor `PATH` setzen. Es akzeptiert nur
...
- [x] Fetch-Fixtures mit E-Mail, Token, Bearer/JWT, URL-Userinfo, Private-Key-Block und
...
- [x] Den vollständigen `fetch -> stage -> approve`-Pfad testen: `stage` bleibt außerhalb des
...
- [x] `tests/spec/brain-k4-brain-wiki/brain-mcp-server.bats` bevorzugt erweitern, statt ein
...
- [x] `tools/list` strukturell prüfen: genau zwei Einträge und als sortierte Namen exakt
  ...
  zulässig.
- [x] Lokale Wiki-Seiten mit verschiedenen Typen, Tags, Status, Quellenarten und festen
...
- [x] Treffer müssen vorhandene Provenienz-/Gültigkeitsfelder unverändert zurückgeben und die
...
- [x] `tests/fixtures/brain/retrieval-eval.jsonl` als `schema_version: 1`-Evalset mit festen
...
- [x] `tests/spec/brain-k4-brain-wiki/retrieval-eval.bats` mit einem vollständig lokalen Wiki-
...
- [x] Durch ein leeres Stub-Verzeichnis vor `PATH` beziehungsweise explizite Netzwerk-Stubs
...
- [x] Beobachtenden Baseline-Vertrag prüfen: Text und JSON berichten Metriken, aber kein
...
- [x] Alle neuen und erweiterten Bundles vollständig ausführen:
...
- [x] Das Testinventar über den autoritativen Generator aktualisieren und das erzeugte
