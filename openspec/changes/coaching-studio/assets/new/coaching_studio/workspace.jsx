...
// Zwischenablage (leert nach Senden + Ebenenwechsel) · Eingabe + Mic-Zustände ·
// Transkriptions-Review · KI-Antwort · Übersetzungs-Panel (DE ∥ Zielsprache, TTS).
// useState/useRef kommen global aus data.jsx.
...
function ResetSwitch({ isDefault, onReset }){
  // "Standard" an = Prompt entspricht Default. Aus = bearbeitet.
  // Klick bei "aus" setzt auf Standard zurück.
  return (
    <button
...
function TranslationPanel(){
  const [lang, setLang] = useState(TARGET_LANGS[0]);   // Farsi default (RTL)
...
function Workspace({ customer, onNav }){
  ...
  const [mic, setMic] = useState("idle");                 // idle · recording · review
  ...
  // Ebenenwechsel: Zwischenablage leeren, Mic/Transkription zurücksetzen.
  const switchLevel = (i)=> {
    if(i===active) return;
  ...
  // Senden: Antwort setzen (Platzhalter), Zwischenablage leeren, Eingabe/Mic reset, Ebene als bearbeitet markieren.
  const send = ()=> {
  ...
  // Mic-Zustände (nur UI): idle → recording → review
  const micClick = ()=> {
  ...
  // Tastaturnavigation der Ebenen-Liste (Pfeil hoch/runter)
  const railKey = (e)=> {
        ...
        <header className="ws-mhead">
          ...
          <div className="who">
            ...
            <div className="row" style={{justifyContent:"flex-end", marginTop:10, gap:8}}>
              ...
              <button className="btn btn-ghost btn-sm" onClick={()=> window.open("Praesentation.html","_blank")} title="Präsentationsfenster (Zweitmonitor)"><Icon.present width="14" height="14"/>Präsentation</button>
