...
- [ ] **`gptoss-context` und `devstral-quality` deaktivieren.** Je `"enabled": false` im Eintrag
      in `scripts/llm/loadouts.json`. Die Einträge bleiben **vollständig** stehen — Ports,
      Modellpfade, `fit`-Werte, alles.

- [ ] **`notes` um die Begründung ergänzen, nicht ersetzen.** Die vorhandenen Messwerte bleiben
...
- [ ] **`brain-ingest` NICHT anfassen — außer beim Reasoning.** Das Loadout fährt dieselbe
...
- [ ] **`qwen3-coder-30b` unverändert lassen.** Weder aktivieren noch abschalten — es lauscht
      ohnehin nicht auf `:8094`. Ein `enabled: false` dort wäre eine Aussage, die dieses Ticket
      nicht belegt hat.

- [ ] **Migration für die Proxy-Backends.** `scripts/migrations/2026-08-10-disable-gptoss-devstral.sql`
...
- [ ] **Vier Agent-Definitionen umhängen** (`.opencode/agent-models.jsonc`):
...
- [ ] **Beschreibungen mitziehen.** Die `description`- und `name`-Felder nennen Modell, Quant,
...
- [ ] **Agent-Namen erwägen, nicht automatisch ändern.** Die Subagenten heißen `gptoss` und
...
- [ ] **`gemma12-vision` als Agent-Ziel aufnehmen.** 262.144 ctx bei 137–149 tok/s und das
...
- [ ] **Permission-Liste des deepseek-Orchestrators prüfen** (~Z. 486–498). Sie nennt die
