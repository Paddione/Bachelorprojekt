...
- [ ] Vor der Implementierung die drei vorhandenen Mess-Skripte lesen und ihre
...
- [ ] Verifikation, dass alle drei existieren und ausfuehrbar sind (Positiv-Anker):
      ```bash
...
- [ ] `scripts/llm/bench-engine-ab.sh` anlegen mit `set -euo pipefail`,
      `REPO_ROOT="$(git rev-parse --show-toplevel)"`, `cd "$REPO_ROOT"`.
- [ ] CLI: `--tag <name>` (Pflicht, geht in Dateinamen und Log-Zeilen ein —
...
- [ ] Mess-Konvention T002717 fest im Skript verankern — JEDE Ausgabezeile
      ...
      ```
- [ ] Ohne `--tag`: `echo "bench-engine-ab.sh: --tag <name> ist Pflicht" >&2; exit 2`.
...
- [ ] Helper `wait_port_free <port> <timeout_s>` implementieren: pollt per
...
- [ ] Ablauf: `stop_freetoken; stop_llamacpp` (Phase 1 startet mit beiden
...
- [ ] `powershell.exe -NoProfile -ExecutionPolicy Bypass -File scripts/llm/start-gptoss-server.ps1`
...
- [ ] `bash scripts/llm/bench-decode.sh 8097 "llamacpp-gptoss20b-${TAG}"`
...
- [ ] `bash scripts/llm/bench-concurrent.sh 8097 1 "llamacpp-gptoss20b-${TAG}-np1"`
...
- [ ] `powershell.exe -NoProfile -ExecutionPolicy Bypass -File scripts/llm/restart-freetoken.ps1 -Profile gptoss-65k -Tag "ab-${TAG}"`
...
- [ ] Nach Bereitschaft `--warmup-requests` (Default 5) sequentielle
      ...
      ```
- [ ] `bash scripts/llm/bench-freetoken-prefill.sh --tag "freetoken-gptoss20b-${TAG}" --base-url http://127.0.0.1:1919`
      ausfuehren und Ausgabe nach
      `scripts/llm/measurements/raw/${TAG}-freetoken-prefill.log` spiegeln.
- [ ] `--max-running-requests 1` (aus dem `gptoss-65k`-Profil, nicht
...
- [ ] `stop_freetoken` (siehe T3).

## Task T6 — Zusammenfuehrung ohne Messlogik-Nachbau

- [ ] Am Skriptende NUR die bereits von den Unterskripten berechneten
      ...
      ```
- [ ] Diese Zusammenfassung zusaetzlich nach
...
- [ ] Nach T6: die Zusammenfassung manuell (kein automatisierter Vergleich,
      ...
      - Nein → Zeile `ABBRUCHPUNKT-2: nicht erreicht` ergaenzen, P5 laeuft
        wie geplant.
- [ ] Diese Entscheidung wird in P4 nur **markiert**, nicht vollzogen — ob P5
...
- [ ] Syntax-Check (kein Interpreter-Fehler vor dem ersten Live-Lauf):
      ```bash
      bash -n scripts/llm/bench-engine-ab.sh
      ```
- [ ] S1-Zeilenlimit einhalten (Budget 800/800, siehe Kopf dieser Datei):
      ```bash
...
- [ ] Funktionaler Lauf auf dem GPU-Host (RTX 5070 Ti, WireGuard-Mesh
      ...
      ```
- [ ] S4-Erreichbarkeit: `bench-engine-ab.sh` wird NICHT von einem
