...
Eliminate at least 90 explicit `any` type annotations from the `website/src` production codebase
(specifically the API-route and shared-lib layers), reducing the total count from the Batch 1
start baseline of 463 to ≤ 373. This is the first of three planned batches toward the overall
G-CQ02 milestone target of ≤ 280. Each eliminated `any` restores TypeScript's ability to catch
shape mismatches, missing properties, and incorrect return types in code that handles HTTP
