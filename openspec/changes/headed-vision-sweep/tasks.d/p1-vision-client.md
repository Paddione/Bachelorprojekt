...
- [ ] **Konfiguration aus der Umgebung lesen.** Eine Funktion `visionConfig()` liefert Endpunkt,
...
- [ ] **Erreichbarkeit einmalig prüfen.** `probeVision()` holt `/v1/models` vom Proxy und stellt
...
- [ ] **Anfrage bauen.** `buildVisionRequest({ route, brand, viewport, domStatus, imageBase64 })`
...
- [ ] **Antwortform erzwingen.** Die Anfrage trägt `response_format` mit dem JSON-Schema aus
...
- [ ] **Antwort auswerten.** `parseVerdict(raw)` prüft die Antwort gegen das Schema und liefert
...
- [ ] **Aufruf kapseln.** `judgeScreenshot(...)` verbindet die Teile, setzt
