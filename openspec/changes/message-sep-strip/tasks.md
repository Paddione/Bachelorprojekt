...
- [ ] **Failing-Test-Step (RED).** `scripts/llm-proxy/strip-markers.test.mjs` anlegen. Prüft
...
- [ ] **Strip-Funktion (GREEN, Teil 1).** `scripts/llm-proxy/strip-markers.mjs` mit
...
- [ ] **Non-Streaming-Pfad im Proxy (GREEN, Teil 2).** In `proxyV1` (`server.mjs:123`) den
...
- [ ] **Fehlerrobustheit (GREEN, Teil 3).** Schlägt das Parsen fehl (kein JSON, abgeschnittener
...
- [ ] **Live-Gegenprobe.** Gegen den laufenden Proxy dieselben fünf Prompts aus dem Ticket
