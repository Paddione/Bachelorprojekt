...
- [ ] **5. llm_proxy_request_log verliert erfolgreiches Dispatch (Blind Spot bei Incident-Analyse)** (suspicious, llm-proxy/request-log) — **OFFEN** — Die genannte Fix-Richtung ist exakt als Ticket T013540 im Backlog dispatcht ("[switch]-Events mit requested_model + Client-Info loggen, Request-Log-Verlust sichtbar machen"). Hier doppelt zu implementieren wäre Duplikatarbeit und Konfliktrisiko; der Eintrag geht mit T013540 geschlossen werden.
