...
- [x] **1. export_ticket_timeline schlägt für Brand korczewski fehl (Exit 3, keine Diagnose)** (degraded, ticket-mcp/export-timeline) — Disposition: _beobachten (bis Zyklus 2026-09-22)_ + Begruendung: Der Fix sitzt im Go-Wrapper (scripts/ticket-mcp/go/internal/tools/list.go:181 — Runner-stderr/Diagnose durchreichen plus Brand-Support in ticket.sh get-timeline); ein eigener MCP-Eingriff mit eigenem Test sprengt diesen Zyklus. Workaround dokumentiert: Hand-SQL gegen tickets.ticket_comments / tickets.factory_phase_events (Spalte 'at', nicht 'created_at').
- [x] **2. Verwaiste Factory-Test-Fixtures akkumulieren ohne Sweeper — geleakte SF-REAL-Zeilen sind in der echten Queue dispatchbar** (degraded, tests/factory-fixtures) — Disposition: _beobachten (bis Zyklus 2026-09-22)_ + Begruendung: Akute Bereinigung erledigt (purge_real_feature/purge_factory_test_data, 17/17 laut Eintrag). Ein altersbasierter Sweeper im Factory-Tick (is_test_data=true > N Tage) ist ein eigener Eingriff mit eigenem Test; bis dahin traegt der Eintrag das Ablaufdatum und wird fortgefuehrt.
- [x] **3. Plan-Status-Flip uncommittet im Hauptcheckout statt im PR (T013593 tasks.md)** (suspicious, factory/post-merge-finalize) — Disposition: _gefixt_ + Begruendung: openspec/changes/archive/2026-08-22-brain-ingest-loadout-swap/tasks.md traegt jetzt status: completed (Flip nachgezogen in diesem Zyklus). Kein RED-Test: reine Archiv-Buchhaltung ohne ausfuehrbares Verhalten; Beleg ist der grep auf die Datei. Das uebergeordnete Muster (Buchhaltungsschreibvorgaenge im falschen Checkout) ist unter T012966/T012968 erfasst.
- [x] **4. Uncommittete .opencode/package.json+-Lockfile-Änderungen im mishap-rollup Worktree T013316-reuse** (suspicious, factory/worktrees) — Disposition: _kein Repo-Fix_ + Begruendung: Die Abweichungen existieren nicht mehr — `git -C .worktrees/mishap-incident-rollup-2026-08-22-T013316-reuse status --porcelain` ist leer (2026-08-22 20:44, Stand c461c40c2); der Worktree wird mit diesem Zyklus abgeraeumt. Transientes npm-Artefakt einer Session, nicht wiederholungsanfaellig ueber die Lebensdauer des Worktrees hinaus.
- [x] **5. worktree-create.sh: Skill-Text zeigt Aufruf ohne Pflicht-Pfadargument** (drift, skills/dev-flow-plan) — Disposition: _gefixt_ + Begruendung: .claude/skills/dev-flow-plan/SKILL.md nennt jetzt die Zwei-Argument-Form (`scripts/worktree-create.sh <branch> <path>`). Guard: tests/spec/dev-flow-plan/skill-worktree-create-aufruf.bats (RED belegt, jetzt gruen).
- [x] **6. freshness:regenerate erzeugt repo-index.json, plan-preflight verbietet sie im selben Zug** (suspicious, skills/dev-flow-plan) — Disposition: _gefixt_ + Begruendung: scripts/plan-preflight.sh nennt in der Fremd-Datei-Meldung die Abhilfe `git restore docs/code-quality/repo-index.json` und kennzeichnet sie als regeneriertes Nicht-Freshness-Artefakt (kein Guard-Defekt). Guard: tests/spec/dev-flow-plan/plan-preflight-repo-index-hint.bats (RED belegt, jetzt gruen).
- [x] **7. components/website/node_modules unvollstaendig im Haupt-Checkout — pg toter Symlink, tsx ohne dist/** (degraded, components/website) — Disposition: _bereits gefixt_ + Begruendung: Lokal repariert (CI=true pnpm install --frozen-lockfile in components/website, 2026-08-22; tsx/dist/cli.mjs und pg-Symlink wieder intakt, portal-profile-update.bats 4/4 gruen) und die strukturelle Abdeckungsluecke (still skippende Tests) ist via T013674 (PR #5019, Squash e69640d58 auf main) geschlossen.
- [x] **8. plan-qa-check.sh uebersprungen — llm-proxy antwortet 503 no_backend** (degraded, scripts/llm-proxy) — Disposition: _beobachten (bis Zyklus 2026-09-22)_ + Begruendung: Bekanntes Discovery-Fenster nach Loadout-Swap (llama-server gesund, /v1/models entscheidet); Workaround dokumentiert. Wiederholungsanfaellig, daher Ablaufdatum statt Repo-Fix in diesem Zyklus.
- [x] **9. Merge=Closure griff nicht bei MCP-angelegtem Chore-Ticket (T013675 blieb triage nach gemergtem PR)** (suspicious, tickets/merge-closure) — Disposition: _beobachten (bis Zyklus 2026-09-22)_ + Begruendung: T013675 wurde manuell auf done/fixed geschlossen (Beleg). Ein Fix am Closure-Pfad (scripts/factory/auto-close-merged.sh) braucht einen eigenen Test gegen den Post-Merge-Pfad und sprengt diesen Zyklus; der Eintrag laeuft bis zum Ablaufdatum fort.
- [x] **10. git-worktree-health.sh objects wertet harmloses dangling als BEFUND (Exit 1)** (degraded, scripts/git-worktree-health.sh) — Disposition: _kein Repo-Fix_ + Begruendung: Gegenprobe widerlegt den Eintrag — der Code (seit #4244 unveraendert) unterscheidet harte fsck-Fehler (rc!=0 → BEFUND) von dangling (rc=0 → HINWEIS, exit 0). Heutiger Lauf: `bash scripts/git-worktree-health.sh objects` → rc=0, HINWEIS. Der beobachtete Exit 1 war ein transientes fsck-Ereignis (GC-Futter nach Squash-Merges/Reaper-Laeufen), nicht wiederholungsanfaellig; kehrt er wieder, ist die RETTUNGSSEQUENZ der korrekte Pfad.

- [x] **Failing-Test-Step (RED).** Erledigt fuer die zwei Code-Fixes: skill-worktree-create-aufruf.bats und plan-preflight-repo-index-hint.bats waren vor den Fixes rot (Beleg: erster Lauf `not ok`), danach gruen. Eintrag 3 ist reine Archiv-Buchhaltung ohne Test (begruendet oben).
