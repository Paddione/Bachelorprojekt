...
- [ ] **Task 1: Failing-Test bestaetigen (RED).** Der Guard liegt schon im Branch.
...
- [ ] **Task 2: Fix (GREEN).** In `scripts/llm/start-qwen-server.ps1` die Zeile `"-ngl", "999",` aus dem `$Params = @( ... )`-Block entfernen und im `-Ctx`-Zweig ergaenzen:
...
- [ ] **Task 3: Kommentar im Skriptkopf korrigieren.** Die Messtabelle entstand mit abgebrochenem `-fit`. Einen Hinweis mit der neuen Messung ergaenzen (205.056 ctx, frei 1752/2782 MiB, ~31 t/s, 2026-09-15) und die `-ngl`-Falle mit der Log-Zeile benennen.

- [ ] **Task 4: Live-Messung.** Skript ohne Parameter starten, Log und VRAM pruefen:
