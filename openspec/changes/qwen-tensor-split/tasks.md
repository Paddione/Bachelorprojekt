...
- [ ] **Task 1: Failing-Test bestaetigen (RED).**
...
- [ ] **Task 3: Loadout (GREEN, Teil 2).** In `scripts/llm/loadouts.json` beim Slug `qwen38-220k` `"-ts", "85,15"` an `extraArgs` anhaengen und die Notiz um die Split-Messung ergaenzen. Format pruefen:
...
- [ ] **Task 4: Live-Messung.** Skript ohne Parameter starten und pruefen: `-ts 85,15` in der Kommandozeile, `n_ctx_slot` 205.056, 3060 Ti mindestens 1500 MiB frei, Decode um 38 t/s.
