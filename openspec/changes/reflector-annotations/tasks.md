...
- [x] **Failing-Test-Step (RED).** Der Guard-Test
...
- [x] **Fix-Step (GREEN).** Aufgaben 1–4 umsetzen; der Test muss danach
      vollständig grün sein.

- [x] **Final Verification.** Die drei Pflicht-CI-Gates plus Manifest-Validierung:
...
- [x] 1.1 Guard-Test laufen lassen und das erwartete Fehlschlagen von Test 3
...
```
      ...
      Festgehalten (2026-08-13): Test 3 rot — Treffer in
      `prod/wildcard-certificate.yaml` und
      `prod-fleet/staging/wildcard-certificate.yaml`.

### 2. `prod/wildcard-certificate.yaml` bereinigen

- [x] 2.1 Den kompletten `secretTemplate`-Block (Zeilen 9–13: die vier
...
- [x] 2.2 Kommentarzeile oberhalb von `spec:` ergänzen, die die reale Mechanik
...
- [x] 3.1 Denselben `secretTemplate`-Block (Zeilen 8–13, mit
      `${WEBSITE_NAMESPACE}`-Werten) entfernen.
- [x] 3.2 Gleichen Dokumentations-Kommentar wie in Aufgabe 2 ergänzen.

### 4. Stale Kommentar in `k3d/coturn-stack/coturn-cert.yaml` korrigieren

- [x] 4.1 Zeilen 1–2: „reflected via reflector (emberstack)" durch den Verweis
...
- [x] 5.1 Guard-Test erneut ausführen — alle drei Tests müssen grün sein
...
- [x] 6.1 Die drei Pflicht-Gates ausführen:
...
- [x] 6.2 Manifest-Validierung (Entwicklungsregel 5):

```bash
task workspace:validate
```

- [x] 6.3 PR eröffnen und mergen (squash) — Ticket wird bei Merge automatisch
