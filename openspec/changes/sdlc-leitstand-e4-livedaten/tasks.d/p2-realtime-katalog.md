...
- [x] **`api/factory-floor/stream.ts` auf LISTEN/NOTIFY umstellen.** Statt
...
- [x] **`api/mcp-health.ts` anlegen (server-seitiger Health-Proxy).** Admin-gated Route
...
- [x] **`ApiKatalog.svelte` anlegen.** Statischer Import von
...
- [x] **`DeckWissen.svelte` umbauen.** Die E3-„API-Katalog-Kachel" durch das vollwertige
