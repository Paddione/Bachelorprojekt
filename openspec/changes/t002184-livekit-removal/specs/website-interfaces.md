...
### Requirement: Live- und Stream-Routen im Portal und Admin-Bereich

**Reason:** `/portal/stream` and `/admin/live` exist only to drive the LiveKit stack, which is
removed by T002184. The DNS pinning endpoint `/api/admin/ops/dns/pin` served the same feature.

**Migration:** Delete `website/src/pages/portal/stream.astro`,
`website/src/pages/admin/live/index.astro`, `website/src/pages/api/admin/ops/dns/pin.ts` and
the components under `website/src/components/live/` and `website/src/components/LiveStream/`.
Remove the admin navigation entries and the portal tiles pointing at them, then regenerate the
route manifest so the removed routes disappear from the inventory. Visitors reaching the old
URLs receive the standard 404 page; no redirect is provisioned because the feature has no
successor.
