...
- [x] **finalize Schritt 10.** Bei den anderen Lib-Einbindungen (um Zeile 110–116):
...
- [x] **`scripts/pr-refresh.sh` (vier Stellen, Zeilen ~189/203/217/233).** Lib einmal oben
...
- [x] **`scripts/weekly-dep-schema-audit.sh` (Zeilen ~61 und ~94).** Lib sourcen, beide
...
- [x] **`scripts/factory/cleanup.sh`.** Lib sourcen (`$(dirname "$0")/../lib/worktree-remove.sh`).
