...
`docs/code-quality/repo-index.json`) by
...
`components/website/package-lock.json` SHALL NOT exist; `components/website/pnpm-lock.yaml` SHALL exist.

#### Scenario: G-CI01-B: Dockerfile COPY-Zeile referenziert pnpm-lock.yaml *(BATS)*

- **GIVEN** `components/website/Dockerfile` ist vorhanden
...
- **THEN** existiert `components/website/pnpm-lock.yaml` und `components/website/package-lock.json` existiert NICHT

---

### Requirement: website ESLint fail-closed gate stays enforced

The `website/` ESLint flat config (`components/website/eslint.config.js`) SHALL set
`@typescript-eslint/no-explicit-any` and `@typescript-eslint/no-unused-vars` to `error`
severity, and `components/website/package.json`'s `lint`/`lint:fix` scripts SHALL invoke ESLint with
...
- **GIVEN** `components/website/package.json` is checked out
...
- **GIVEN** `components/website/eslint.config.js` is checked out
...
- **WHEN** `pnpm --prefix components/website lint` runs
...
- **GIVEN** the suite `tests/spec/software-factory/`, whose `@test` titles carry the
  structured IDs `FA-SF-01` through `FA-SF-74`
- **WHEN** the inventory is regenerated
- **THEN** the inventory still contains exactly 54 entries for that suite
...
4. every file whose blob hash differs from `origin/main` matches the allowlist of plan and
   ...
   `.release-please-manifest.json`, `components/website/CHANGELOG.md`, `components/website/package.json`).
...
Website-Image, weil `components/website/src/lib/sdlc/goals-data.generated.json` per statischem ESM-Import in
`components/website/src/lib/sdlc/goals-data.ts` ins Astro-Bundle gebacken wird. `.claude/lib/goals.md` ist der
...
- **GIVEN** the mishap rollup driver has generated and linted a plan
...
- **GIVEN** `components/website/Dockerfile` enthält einen `--max-old-space-size=<N>`-Eintrag
...
- **GIVEN** `components/website/Dockerfile` hat einen Build-Stage-Marker und einen Runtime-Stage-Marker
...
<!-- merged from change delta ci-cd.md (f80639f39bd5) -->

<!-- merged from change delta ci-cd.md (a1f1998eaa5b) -->

<!-- merged from change delta ci-cd.md (7665da4334b5) -->

<!-- merged from change delta ci-cd.md (882facf285e4) -->

<!-- merged from change delta ci-cd.md (d5db22597671) -->

<!-- merged from change delta ci-cd.md (60ba711c8b93) -->

<!-- merged from change delta ci-cd.md (3305ce922892) -->
