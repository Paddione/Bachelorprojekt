...
### Requirement: Der Measure-Command `grep -rnE "(describe|it|test)\

The system SHALL der Measure-Command `grep -rnE "(describe|it|test)\.(skip|todo)\b" website/src --include="*.ts" --include="*.svelte" | grep -vE "^[^:]+:[0-9]+:[[:space:]]*//" | wc -l` ist reproduzierbar und liefert in der CI-Umgebung dieselben Ergebnisse wie lokal.
...
<!-- merged from change delta g-test03-vitest-skip-todo.md on 2026-06-28 -->