...
#### Scenario: Syntaxprüfung des OpenClaw-Taskfiles
...
#### Scenario: Alle Pflicht-Tasks sind vorhanden
...
- **THEN** jeder Task ist als Top-Level-Eintrag in der Form `  <name>:` deklariert
- **AND** kein Task fehlt; andernfalls wird der fehlende Task-Name als Fehlermeldung ausgegeben

#### Scenario: Kein undokumentierter Task-Name

- **GIVEN** ein neuer Task soll zum OpenClaw-Taskfile hinzugefügt werden
- **WHEN** der Task-Name in die Datei eingetragen wird
- **THEN** der Task ist mit `  <name>:` als gültiger Eintrag auf Task-Ebene greifbar
- **AND** der Test erkennt den neuen Task beim Durchlauf der Pflicht-Liste
...
#### Scenario: Korrekte Base-URL für lokales Ollama
...
#### Scenario: Chat-Modell ist gesetzt
...
#### Scenario: Einbindung im Root-Taskfile
...
#### Scenario: `.env` ist in `.gitignore` eingetragen
