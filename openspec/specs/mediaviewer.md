...
#### Scenario: Prod-ConfigMap enthält MEDIAVIEWER_HOST

- **GIVEN** das Prod-Overlay `prod/configmap-domains.yaml` wird auf den Base-ConfigMap `k3d/configmap-domains.yaml` gemerged
- **WHEN** Kustomize den strategischen Merge anwendet
- **THEN** enthält der resultierende `domain-config` ConfigMap einen expliziten `MEDIAVIEWER_HOST`-Eintrag, der den Dev-Wert überschreibt

#### Scenario: Dev-Base bleibt unverändert (Patch-only-Strategie)

- **GIVEN** `k3d/configmap-domains.yaml` enthält `MEDIAVIEWER_HOST: "mediaviewer.localhost"` als Dev-SSOT
- **WHEN** ausschließlich das Prod-Overlay angepasst wird
- **THEN** bleibt der Base-ConfigMap unverändert bei `mediaviewer.localhost` — Prod überschreibt per Patch, ohne die Base zu editieren
...
#### Scenario: PROD_DOMAIN-Platzhalter im Prod-ConfigMap

- **GIVEN** `prod/configmap-domains.yaml` wird geprüft
- **WHEN** der `MEDIAVIEWER_HOST`-Eintrag ausgelesen wird
- **THEN** hat er die Form `"mediaviewer.${PROD_DOMAIN}"` — eine envsubst-Variable, kein fest kodierter Hostname

#### Scenario: PROD_DOMAIN in der envsubst-Variablenliste des Deploy-Tasks

- **GIVEN** der Deploy-Task führt `envsubst "$ENVSUBST_VARS"` auf die Kustomize-Ausgabe aus
- **WHEN** `ENVSUBST_VARS` ausgewertet wird
- **THEN** ist `PROD_DOMAIN` in dieser Liste enthalten, sodass `mediaviewer.${PROD_DOMAIN}` korrekt substituiert wird und nicht literal im Cluster landet
...
#### Scenario: ConfigMapKeyRef-Referenz im Website-Manifest

- **GIVEN** das Website-Deployment-Manifest `k3d/website.yaml` wird geprüft
- **WHEN** die Umgebungsvariablen-Sektion des Containers ausgelesen wird
