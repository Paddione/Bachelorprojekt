...
#### Scenario: Alle Pflicht-Alerts deklariert

- **GIVEN** die Datei `k3d/monitoring/prometheus-rules.yaml` existiert
- **WHEN** der Inhalt auf `alert:` Einträge geprüft wird
- **THEN** sind alle 8 Alerts — `PodCrashLoopBackOff`, `HighCPUUsage`, `HighMemoryUsage`, `HighDiskUsage`, `High5xxErrorRate`, `PodRestartSpike`, `NodeHighCPUUsage`, `NodeFilesystemAlmostFull` — vorhanden

#### Scenario: Kein Pflicht-Alert fehlt

- **GIVEN** die Datei `k3d/monitoring/prometheus-rules.yaml` enthält Alert-Definitionen
- **WHEN** ein einzelner Pflicht-Alert (z. B. `NodeFilesystemAlmostFull`) aus der Datei entfernt wird
- **THEN** schlägt die Validierung fehl, weil der erwartete `alert:` Eintrag nicht gefunden werden kann
...
#### Scenario: Valide Prometheus-Regelstruktur
...
#### Scenario: Kein promtool — Test überspringen
...
#### Scenario: Pushover-Empfänger konfiguriert
...
#### Scenario: E-Mail-Empfänger konfiguriert
...
#### Scenario: Keine hardcodierten Markennamen
...
#### Scenario: Kommentare sind ausgenommen

- **GIVEN** die Datei enthält einen YAML-Kommentar mit einem Markennamen (z. B. `# example: mentolder.de`)
- **WHEN** nur nicht kommentierte Zeilen geprüft werden
- **THEN** wird der Kommentar nicht als Verstoß gewertet
...
#### Scenario: Kustomize-Build erfolgreich
...
#### Scenario: Fehlende Ressource bricht den Build

- **GIVEN** eine in der `kustomization.yaml` referenzierte Ressourcendatei fehlt
- **WHEN** `kubectl kustomize k3d/monitoring/` ausgeführt wird
- **THEN** endet der Befehl mit einem Exit-Code ungleich 0 und gibt eine Fehlermeldung aus
