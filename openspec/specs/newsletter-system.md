...
#### Scenario: CronJob im Basis-Kustomize vorhanden
...
#### Scenario: Cron-Schedule auf 5-Minuten-Intervall gesetzt
...
#### Scenario: Timezone explizit im Manifest konfiguriert

- **GIVEN** der CronJob `scheduled-publish` ist im Cluster registriert
- **WHEN** ein Scheduler-Durchlauf um Mitternacht Berliner Zeit ausgelöst wird
- **THEN** wird der Job zur korrekten Berliner Lokalzeit ausgeführt, nicht zur UTC-Zeit
...
#### Scenario: Gleichzeitiger Job-Start wird verhindert
...
#### Scenario: Fehlender oder falscher Bearer-Token wird abgewiesen

- **GIVEN** der API-Endpunkt `scheduled-publish.ts` ist implementiert
- **WHEN** eine Anfrage ohne gültigen Bearer-Token eingeht
- **THEN** antwortet der Endpunkt mit HTTP-Status 401

#### Scenario: Gültiger Bearer-Token wird akzeptiert

- **GIVEN** der API-Endpunkt `scheduled-publish.ts` prüft den Header gegen `Bearer ${CRON_SECRET}`
- **WHEN** eine Anfrage mit dem korrekten Secret im Authorization-Header eintrifft
- **THEN** wird die Anfrage verarbeitet und der Status 401 nicht zurückgegeben
...
#### Scenario: Atomare Lock-Abfrage verhindert Doppelveröffentlichung
...
#### Scenario: Veraltete Sending-Locks werden nach 10 Minuten freigegeben
...
#### Scenario: Korczewski-Patch zeigt auf eigenen Namespace-Endpunkt
