...
The system SHALL protect the `/admin/factory-observability` page behind admin authentication, render an observable dashboard with KPI cards for authenticated admins, and return a JSON response from `/api/factory-observability` with `brand`, `timeline`, and `fetchedAt` fields.

#### Scenario: T1: /admin/factory-observability lädt mit KPI-Cards für Admin *(E2E)*
- **GIVEN** `E2E_ADMIN_PASS` ist gesetzt (Admin-Auth vorhanden)
- **WHEN** `/admin/factory-observability` aufgerufen wird
- **THEN** `.obs-dashboard` und `.kpi-row` sind sichtbar
...
#### Scenario: T3: /admin/factory-observability leitet unauthentifizierte Nutzer weiter *(E2E)*
- **GIVEN** ein Browser ohne Admin-Session
- **WHEN** `/admin/factory-observability` aufgerufen wird
- **THEN** URL ist nicht mehr `/admin/factory-observability` (Redirect)
...
The system SHALL render the Factory Floor dashboard at `/dev-status` with hall sections (Leitstand, Hall, Shipped, Slots) and open a detail panel when a workpiece is clicked.

#### Scenario: Hallen-Sektionen werden gerendert *(E2E)*
- **GIVEN** `/dev-status` ist abrufbar und Admin-Auth ist aktiv
...
The system SHALL render the Factory Floor on mobile viewports (375×812) as a bottom-sheet detail panel with backdrop and ≥44px close button, ensure content padding so the last loading-dock item is not obscured by the tab bar, provide 5 horizontally-scrollable outer tabs, 10 inner mobile-station tabs with dot indicators, and render the Leitstand grid with 8 cards without horizontal overflow.
...
#### Scenario: FA-MOBILE-03: Alle 5 DevStatus-Outer-Tabs via Horizontal-Scroll erreichbar *(E2E)*
- **GIVEN** ein Mobile-Viewport auf `/dev-status`
- **WHEN** `.tab-bar-wrap` horizontal gescrollt und alle 5 `.ds-tab`-Elemente angeklickt werden
- **THEN** jeder Tab bekommt die Klasse `active`
...
#### Scenario: FA-MOBILE-06: Leitstand-Grid — alle 8 Karten ohne Horizontal-Scroll *(E2E)*
- **GIVEN** ein Mobile-Viewport und `[data-testid="floor-leitstand"]` ist sichtbar
- **WHEN** die Leitstand-Breite mit `scrollWidth` vs `clientWidth` verglichen wird
- **THEN** kein horizontaler Overflow; `floor-slots`, `floor-office`, `floor-komm-count` sind sichtbar; `leitstand > *` hat Count 8
