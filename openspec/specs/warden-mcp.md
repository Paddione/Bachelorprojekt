...
Agenten-Sessions brauchen regelmäßig Zugangsdaten aus der Vaultwarden-Instanz des Nutzers und
sollen neu erzeugte oder rotierte Secrets direkt dort ablegen können, ohne dass der Nutzer Secrets
per Copy-Paste in den Chat schreibt. Der MCP-Server `warden` (`@icoretech/warden-mcp`, im Launcher
gepinnt) kapselt die offizielle Bitwarden-CLI, hält Secrets standardmäßig redacted und läuft als
lokaler stdio-Server über `scripts/warden-mcp/launch.mjs`. Credentials stehen ausschließlich in
`~/.config/warden-mcp/server.env`; jedes mutierende Tool verlangt in beiden Harness-Configs
(Claude Code, opencode) eine Bestätigung.
...
<!-- merged from change delta warden-mcp.md (433fc607814a) -->