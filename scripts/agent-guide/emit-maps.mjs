...
import { loadRegistry, tierFor, toolById, guardrailById, roleById, runtimeById } from './load.mjs';
...
/**
 * Render agents-map.md from the in-memory registry.
 * Two tables: roles (Agent, Zuständigkeit, Claude Code, agy, opencode, Tools)
 * and runtimes (Agent, Modus, Modell, schreibfähig, Hinweis).
 * @param {ReturnType<import('./load.mjs').loadRegistry>} reg
 * @returns {string}
 */
export function renderAgentsMap(reg) {
  const out = [];
  out.push(HEADER);
  out.push('');
  out.push('# Agenten-Karte (Agents Map)');
  out.push('');
  out.push('Übersicht der Claude Code Domain-Agenten (roles) und opencode-Runtimes.');
  out.push('Die Registry ist die SSOT: `docs/agent-guide/registry/agents.yaml`.');
  out.push('');

  const roles = Object.entries(reg.agents?.roles ?? {}).sort((a, b) => a[0].localeCompare(b[0]));
  out.push('## Rollen (Claude Code Domain Agents)');
  out.push('');
  out.push(row(['Agent', 'Zuständigkeit', 'Claude Code', 'agy', 'opencode', 'Tools']));
  out.push(row(['---', '---', '---', '---', '---', '---']));

  const descriptions = {
    bachelorprojekt: {
      db: 'PostgreSQL, Schema, Queries, Backup',
  ...
  for (const [name, role] of roles) {
    const short = name.replace('bachelorprojekt-', '');
    const desc = descriptions.bachelorprojekt[short] || EMPTY;
    const cc = escapeCell(role.claude_code);
    const agy = role.agy === 'unsupported' ? '🚫 unsupported' : escapeCell(role.agy);
    const oc = escapeCell(role.opencode);
    const tools = role.tools ? escapeCell(role.tools.join(', ')) : EMPTY;
    out.push(row([escapeCell(name), desc, cc, agy, oc, tools]));
  }

  out.push('');
  const runtimes = Object.entries(reg.agents?.runtimes ?? {}).sort((a, b) => a[0].localeCompare(b[0]));
  out.push('## Runtimes (opencode Agenten)');
  out.push('');
  out.push(row(['Agent', 'Modus', 'Modell', 'Schreibfähig', 'Hinweis']));
  out.push(row(['---', '---', '---', '---', '---']));
...
export async function emitAll({ registryDir, mapsDir, repoRoot, validate }) {
  ...
  const reg = { ...loadRegistry(registryDir), tierFor, toolById, guardrailById, roleById, runtimeById };
  const roleCount = Object.keys(reg.agents?.roles ?? {}).length;
  const rtCount = Object.keys(reg.agents?.runtimes ?? {}).length;
  const files = [
    ...
    { name: 'agents-map.md', body: renderAgentsMap(reg), count: roleCount + rtCount, unit: `agents (${roleCount} roles + ${rtCount} runtimes)` },
  ];
