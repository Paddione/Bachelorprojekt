...
server.tool('factory_status', 'Show factory queue depth and whether a tick is running', z.object({}), async () => {
...
server.tool('factory_queue', 'List waiting tickets (backlog + plan_staged)', z.object({}), async () => {
...
server.tool('factory_enqueue', 'Enqueue a ticket into the factory backlog', z.object({ ticket_id: z.string().describe('Ticket external_id (e.g. T000123)') }), async ({ ticket_id }) => {
  try {
...
server.tool('factory_trigger', 'Trigger an immediate factory tick (runs wakeup.sh in background)', z.object({}), async () => {
...
server.tool('factory_recent', 'Show last N factory run comments from ticket_comments', z.object({ limit: z.number().optional().describe('Number of recent entries (default 10)') }), async ({ limit }) => {
