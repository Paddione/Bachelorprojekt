...
row target G-FE03 "$(count 'console\.(log|debug|info)' website/src)" le 0 "stray console.log/debug/info"
...
row target G-SEC05 "$(git log -50 --pretty='%G?' main 2>/dev/null | grep -c N)" le 3 "unsignierte Commits (letzte 50)"
