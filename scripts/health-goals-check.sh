...
# Mess-Helfer (Defln. + Variablen-Vorgaben, keine Ausfuehrung) [T900380]: ausgelagert,
# damit die row()-Zeilen das S1-Budget nicht mit Logik aufzehren.
. "$(dirname "${BASH_SOURCE[0]}")/lib/health-goals-measure.sh"
...
row() {
  ...
  # T900380: ein nicht-numerischer Messwert (leer, mehrzeilig, Text) ist KEIN
  # Ergebnis. Ohne diesen Guard fiel er in den `*) ok=0`-Zweig und erschien als
  # 🟡 mit LEERER Zahl — nicht unterscheidbar von einer echten Verletzung. Genau
  # das war der stille Ausfall von runtime_measure (T013916-Muster 3): alle 15
  # Ziele meldeten "unerfuellt", ohne dass ein Messfehler sichtbar wurde.
  case "$actual" in
    ''|*[!0-9]*)
      printf "  WARNUNG: %s liefert einen nicht-numerischen Messwert ('%s') — als n/a gezaehlt, nicht als Verletzung\n" \
        "$id" "$(printf '%s' "$actual" | tr '\n' ' ')" >&2
      SKIP=$((SKIP+1))
      [ "$QUIET" = 0 ] && printf "  %s %-9s %s%-22s%s %s\n" "·" "$id" "$C_D" "n/a" "$C_X" "$desc (Messwert unbrauchbar)"
      return ;;
  esac
  case "$cmp" in
...
# [T900380] 20 -> 300: "5/Woche" gegen ein Ist von ~546 war um Faktor 27 zu locker und
# konnte nicht rot werden. Der Ratchet sitzt jetzt bei 55 % des Ist.
row target G-DORA01 "$(dora_count '4 weeks ago')" ge 300 "DORA Deployment Frequency (main-Merges/4 Wochen, Ziel 75/Woche)"
# [T900380] Neu definiert: der Proxy zaehlte `^fix`-Commits (Fehlerbehebungen, keine
# Fehlschlaege) und stand bei 27 %, waehrend G-DORA04 fuer denselben Zeitraum 0
# Reverts/Hotfixes zaehlte — derselbe Sachverhalt, zwei widerspruechliche Ampelstaende.
# Jetzt derselbe Ereignistyp wie G-DORA04, aber skalenfrei als Rate. Promille statt Prozent,
# damit 0.5 % exakt darstellbar ist (ganzzahliges Prozent wuerde 0.4 % und 0.5 % beide
# auf 0 abrunden und die Schwelle waere faktisch <= 1 %).
row target G-DORA03 "$(t=$(dora_count '8 weeks ago'); f=$(dora_count '8 weeks ago' 'revert|hotfix'); { [ "$t" = "-" ] || [ "$f" = "-" ] || [ "$t" -eq 0 ]; } && echo '-' || echo $(( f * 1000 / t )))" le 5 "DORA Change Failure Rate (revert/hotfix-Anteil in Promille)"
