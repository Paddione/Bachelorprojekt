...
for var in NEXTCLOUD_OIDC_SECRET \
           ...
           DOCS_OIDC_SECRET TRAEFIK_OIDC_SECRET MAIL_OIDC_SECRET BRETT_OIDC_SECRET \
           COMFY_OIDC_SECRET GRAFANA_OIDC_SECRET \
           MEDIAVIEWER_OIDC_CLIENT_SECRET VIDEOVAULT_OIDC_SECRET \
           RECOVERY_OIDC_SECRET DEV_WORKSPACE_OIDC_SECRET \
           SMTP_FROM SMTP_HOST SMTP_USER SMTP_PASSWORD SMTP_PORT \
           PROD_DOMAIN NC_DOMAIN WEB_DOMAIN VAULT_DOMAIN DOCS_DOMAIN TRAEFIK_DOMAIN MAIL_DOMAIN \
           RECOVER_DOMAIN DEV_DOMAIN; do
  eval val="\${${var}:-}"
  if [ -z "$val" ]; then
