...
const EnvSchema = v.object({
  ...
  // Optional user / credential overrides for split password or alias setups
  MAILBOX_IMAP_USER: v.optional(v.pipe(v.string(), v.minLength(1))),
  MAILBOX_IMAP_PASSWORD: v.optional(v.pipe(v.string(), v.minLength(1))),
  MAILBOX_SMTP_USER: v.optional(v.pipe(v.string(), v.minLength(1))),
  MAILBOX_SMTP_PASSWORD: v.optional(v.pipe(v.string(), v.minLength(1))),
  MAILBOX_CALDAV_USER: v.optional(v.pipe(v.string(), v.minLength(1))),
  MAILBOX_CALDAV_PASSWORD: v.optional(v.pipe(v.string(), v.minLength(1))),
  MAILBOX_CARDDAV_URL: v.optional(v.pipe(v.string(), v.url())),
  MAILBOX_FROM_EMAIL: v.optional(v.pipe(v.string(), v.email())),
  MAILBOX_FROM_NAME: v.optional(v.pipe(v.string(), v.minLength(1))),
...
export function loadConfig(): ServerConfig {
  try {
    ...
    const imapUser = validatedEnv.MAILBOX_IMAP_USER || email;
    const imapPassword = validatedEnv.MAILBOX_IMAP_PASSWORD || password;
    const smtpUser = validatedEnv.MAILBOX_SMTP_USER || imapUser;
    const smtpPassword = validatedEnv.MAILBOX_SMTP_PASSWORD || imapPassword;
    const caldavUser = validatedEnv.MAILBOX_CALDAV_USER || email;
    const caldavPassword = validatedEnv.MAILBOX_CALDAV_PASSWORD || password;
    ...
    return {
      ...
      smtp: {
        ...
        fromAddress: validatedEnv.MAILBOX_FROM_EMAIL || email,
        fromName: validatedEnv.MAILBOX_FROM_NAME,
      },
      calendar: {
