...
class MailboxMcpServer {
  ...
  private initializeServices(): void {
    try {
      ...
      this.cardDavService = new CardDavService(this.config.calendar);
  ...
  private setupToolHandlers(): void {
    this.server.setRequestHandler(ListToolsRequestSchema, () => {
      ...
      const contactTools = createContactTools(this.cardDavService);
      const sieveTools = getSieveTools();

      return {
        tools: [...emailTools, ...calendarTools, ...contactTools, ...sieveTools],
      };
    });
    ...
    this.server.setRequestHandler(CallToolRequestSchema, async request => {
      ...
      try {
        ...
        if (isEmailTool(name)) {
          ...
          result = await handleContactTool(
            this.cardDavService,
            name,
            cleanArgs,
          );
