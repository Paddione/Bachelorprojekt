...
export async function runMigrations(pool, migrationsDirOverride = null) {
  ...
  try {
    ...
    if (!existsSync(targetDir)) {
      console.warn(`[migrate-db] migrations dir missing at ${targetDir} — skipping`);
      return;
    }
    ...
    for (const f of files) {
      if (applied.has(f)) {
        console.log(`[migrate-db] ${f}: already applied — skipping`);
        continue;
      }

      const sql = readFileSync(join(targetDir, f), 'utf8');
      console.log(`[migrate-db] ${f}: applying`);
      try {
        ...
        if (isPgError(e) && ALREADY_EXISTS_SQLSTATES.has(e.code)) {
          console.log(
            `[migrate-db] ${f}: already applied (backfill ${e.code}) — tracking`,
          );
          await client.query(
...
export const runFactoryMigrations = runMigrations;

async function main() {
  const args = process.argv.slice(2);
  if (args.includes('--help') || args.includes('-h')) {
    console.log(`Usage: node scripts/migrate-db.mjs [options]
...
if (isMain) {
  main().catch((e) => {
    console.error('[migrate-db] failed:', e);
    process.exit(1);
  });
}
