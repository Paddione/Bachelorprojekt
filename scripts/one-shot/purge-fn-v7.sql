-- 2026-08-09 — Migration: tickets.fn_purge_test_data() v7 (T002781).
--
-- Adds an is_test_data column existence guard on top of v6: the tickets.tickets
-- column is guaranteed to exist (migration 2026-07-08-is-test-data.sql) but the
-- guard makes the function self-documenting and mirrors the pattern already used
-- for every other optional column. Without this guard, a future removal without
...
COMMENT ON FUNCTION tickets.fn_purge_test_data() IS
  'Idempotent test-data purge. v7 (2026-08-09, T002781): adds is_test_data column guard.';
