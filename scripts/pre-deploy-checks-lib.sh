...
check_envsubst_vars() {
  ...
  local ENVSUBST_VARS
  if [[ "$IS_DEV" == "true" ]]; then
    ENVSUBST_VARS="PROD_DOMAIN BRAND_NAME CONTACT_EMAIL BACKUP_ALERT_EMAIL BRAND_ID"
  else
    ENVSUBST_VARS="PROD_DOMAIN BRAND_NAME CONTACT_EMAIL BACKUP_ALERT_EMAIL INFRA_NAMESPACE TLS_SECRET_NAME SMTP_FROM MAIL_FROM_LOCAL MAIL_FROM_DOMAIN WEBSITE_IMAGE TURN_PUBLIC_IP TURN_NODE BRAND_ID KC_USER1_USERNAME KC_USER1_EMAIL KC_USER2_USERNAME KC_USER2_EMAIL BRETT_DOMAIN BRAIN_EXTERNAL_URL"
  fi
