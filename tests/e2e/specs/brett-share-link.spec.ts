...
test.describe('Brett share link (T000608)', () => {
  ...
  test('leader creates a share link; guest views the board read-only', async ({ page }) => {
    ...
    await expect(page.getByText('Link in Zwischenablage kopiert!')).toBeVisible({ timeout: 60_000 });
    ...
    await expect(guest.locator('#view-only-badge')).toBeVisible({ timeout: 60_000 });
    await expect(guest.locator('canvas')).toBeVisible({ timeout: 60_000 });
