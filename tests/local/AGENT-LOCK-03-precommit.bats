...
@test "AGENT-LOCK-03f: real pre-commit hook blocks in main but not in a worktree" {
  ...
  touch "$TMPREPO/.gitleaks.toml"
  chmod +x "$TMPREPO/.githooks/pre-commit" "$TMPREPO/scripts/agent-lock.sh"
  ...
  AGENT_LOCK_SID=200 SKIP_MAIN_COMMIT_GUARD=1 git -C "$TMPREPO" commit -q -m seed
  ...
  AGENT_LOCK_SID=200 AGENT_LOCK_FAKE_ALIVE="100 200" SKIP_MAIN_COMMIT_GUARD=1 run git -C "$TMPREPO" commit -m "blocked"
  [ "$status" -ne 0 ]
