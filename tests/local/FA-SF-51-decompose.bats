#!/usr/bin/env bats
# FA-SF-51: pipeline-decompose — offline unit tests for the decomposition helper.
MOD="scripts/factory/pipeline-decompose.cjs"
SUITE="scripts/factory/pipeline-decompose.test.cjs"

@test "FA-SF-51: pipeline-decompose.cjs exists and is syntactically valid" {
  [ -f "$MOD" ]
  run node --check "$MOD"
  [ "$status" -eq 0 ]
}

@test "FA-SF-51: node --test suite passes" {
  run node --test "$SUITE"
  [ "$status" -eq 0 ]
  [[ "$output" == *"# fail 0"* ]]
}

@test "FA-SF-51: exports the six contract functions" {
...
@test "FA-SF-51: exports via module.exports (CommonJS)" {
  run grep -q "module.exports" "$MOD"
  [ "$status" -eq 0 ]
}

@test "FA-SF-51: defines SHARED_FILE_LIST with the three shared files" {
