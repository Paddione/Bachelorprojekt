...
# T2–T5 depend on a working redirect chain; skip if billing isn't responding
if [[ "$BILLING_OK" == "true" && "$BILLING_STATUS" == "302" ]]; then
  ...
  skip_test "SA-09" "T2" "Billing-SSO Redirect" "billing-Domain nicht erreichbar oder kein 302 (HTTP ${BILLING_STATUS})"
  skip_test "SA-09" "T3" "OAuth2-Proxy Callback" "billing-Domain nicht erreichbar oder kein 302 (HTTP ${BILLING_STATUS})"
  skip_test "SA-09" "T4" "Statische Assets" "billing-Domain nicht erreichbar oder kein 302 (HTTP ${BILLING_STATUS})"
  skip_test "SA-09" "T5" "manifest.json CORS" "billing-Domain nicht erreichbar oder kein 302 (HTTP ${BILLING_STATUS})"
fi
