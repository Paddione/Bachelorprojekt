...
#
# Hintergrund T015008: Der Name des damaligen lokalen k3d-Contexts löste nach einem
...
@test "T015008: guard rejects loopback context server with loud drift error" {
  run env KUBECONFIG="$LOOPBACK_CFG" bash "$GUARD" lab-cluster
...
@test "T015008: guard accepts LAN-resolved context (Positiv-Anker)" {
  run env KUBECONFIG="$LAN_CFG" bash "$GUARD" lab-cluster
...
@test "T015008: escape hatch TICKET_ALLOW_LOCAL_CTX warns instead of aborting" {
  run env KUBECONFIG="$LOOPBACK_CFG" TICKET_ALLOW_LOCAL_CTX=1 bash "$GUARD" lab-cluster
