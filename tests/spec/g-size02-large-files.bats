...
@test "G-SIZE02: VideoVault files >600 lines count is at most 8" {
  run bash -c "find '$REPO_ROOT/components/VideoVault' -type f \( -name '*.ts' -o -name '*.tsx' -o -name '*.js' -o -name '*.jsx' -o -name '*.svelte' -o -name '*.astro' \) ! -path '*/node_modules/*' ! -type l -exec wc -l {} + | awk '\$1 > 600 && \$2 != \"total\"' | wc -l | tr -d ' '"
