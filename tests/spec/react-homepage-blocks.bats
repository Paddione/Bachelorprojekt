...
@test "schema covers all 7 catalog block type literals" {
  run grep -E "z\.literal\('(hero|stats|services|whyMe|process|faq|cta)'\)" \
    "$REPO/components/mentolder-web/src/blocks/schema.ts"
  [ "$status" -eq 0 ]
...
@test "schema covers the 3 generic block type literals" {
  for t in richText image spacer; do
    run grep -E "z\.literal\('$t'\)" "$REPO/components/mentolder-web/src/blocks/schema.ts"
    [ "$status" -eq 0 ]
  done
}

@test "schema exports a single SCHEMA_VERSION constant" {
  run grep -cE "export const SCHEMA_VERSION" "$REPO/components/mentolder-web/src/blocks/schema.ts"
  [ "$status" -eq 0 ]
  [ "$output" -ge 1 ]
  # Must be a literal number (1)
  run grep -E "export const SCHEMA_VERSION = [0-9]+" "$REPO/components/mentolder-web/src/blocks/schema.ts"
  [ "$status" -eq 0 ]
}
...
@test "services.items[].icon is a closed enum of iconRegistry keys" {
  for key in fuehrung digitalisierung team strategie kommunikation resilienz; do
    run grep -E "'$key'" "$REPO/components/mentolder-web/src/blocks/schema.ts"
    [ "$status" -eq 0 ]
  done
}
...
@test "whyMe.props.intro is structured as prefix/emphasis/suffix" {
  for field in prefix emphasis suffix; do
    run grep -E "$field: z\.string\(\)" "$REPO/components/mentolder-web/src/blocks/schema.ts"
    [ "$status" -eq 0 ]
  done
}
...
@test "seed.ts contains all 7 catalog block type literals in order" {
  run bash -c "grep -oE \"type: '(hero|stats|services|whyMe|process|faq|cta)'\" '$REPO/components/mentolder-web/src/blocks/seed.ts'"
  [ "$status" -eq 0 ]
...
@test "seed.ts contains the inline testimonial (quoteName/quoteRole)" {
  run grep "quoteName: 'Gerald Korczewski'" "$REPO/components/mentolder-web/src/blocks/seed.ts"
  [ "$status" -eq 0 ]
  run grep "quoteRole: 'Coach und digitaler Begleiter'" "$REPO/components/mentolder-web/src/blocks/seed.ts"
  [ "$status" -eq 0 ]
}

@test "seed.ts uses structured whyMe intro with prefix/emphasis/suffix" {
  run grep "prefix: 'Ich kenne beide Welten: '" "$REPO/components/mentolder-web/src/blocks/seed.ts"
  [ "$status" -eq 0 ]
  run grep "emphasis: '40 Jahre etablierte Strukturen'" "$REPO/components/mentolder-web/src/blocks/seed.ts"
  [ "$status" -eq 0 ]
  run grep "suffix: ' UND modernste KI-Tools" "$REPO/components/mentolder-web/src/blocks/seed.ts"
  [ "$status" -eq 0 ]
}
...
@test "BlockRenderer validates the document with Zod" {
  run grep "HomepageBlocksDocument" "$REPO/components/mentolder-web/src/blocks/BlockRenderer.tsx"
  [ "$status" -eq 0 ]
  run grep "safeParse" "$REPO/components/mentolder-web/src/blocks/BlockRenderer.tsx"
  [ "$status" -eq 0 ]
}

@test "BlockRenderer falls back to seed on schemaVersion mismatch" {
  run grep "schemaVersion !== SCHEMA_VERSION" "$REPO/components/mentolder-web/src/blocks/BlockRenderer.tsx"
  [ "$status" -eq 0 ]
  run grep "homepageSeed" "$REPO/components/mentolder-web/src/blocks/BlockRenderer.tsx"
  [ "$status" -eq 0 ]
}
...
@test "HomePage.tsx no longer renders inline homepage content (no <Hero direct import)" {
  ! grep -q "from '@/components/Hero'" "$REPO/components/mentolder-web/src/pages/HomePage.tsx"
}

@test "HomePage.tsx no longer imports homepage content fields from content.ts" {
  ! grep -qE "from '@/content'" "$REPO/components/mentolder-web/src/pages/HomePage.tsx" \
    || {
      # If content.ts is imported, only SITE.* (SEO metadata) is allowed
      grep -E "from '@/content'" "$REPO/components/mentolder-web/src/pages/HomePage.tsx" | grep -qE "\{ ?SITE ?\}" \
        && ! grep -E "from '@/content'" "$REPO/components/mentolder-web/src/pages/HomePage.tsx" | grep -qE "(heroContent|stats|services|faqItems|processSteps|whyMe)"
    }
}

@test "HomePage.tsx is reduced to <50 lines (Null-Diff refactor)" {
  lines=$(wc -l < "$REPO/components/mentolder-web/src/pages/HomePage.tsx")
  [ "$lines" -lt 50 ]
}

@test "no block component imports content.ts" {
  for f in $(find "$REPO/components/mentolder-web/src/blocks" -name "*.tsx" -not -name "*.test.tsx"); do
...
@test "vitest config has jsdom environment and @ alias" {
  run grep "jsdom" "$REPO/components/mentolder-web/vitest.config.ts"
  [ "$status" -eq 0 ]
  run grep "find: '@'" "$REPO/components/mentolder-web/vitest.config.ts"
  [ "$status" -eq 0 ]
}

@test "SVG ?react imports are stubbed in vitest config" {
  run grep -E "svg.*react" "$REPO/components/mentolder-web/vitest.config.ts"
  [ "$status" -eq 0 ]
  [ -f "$REPO/components/mentolder-web/src/test/svg-stub.tsx" ]
}
