...
# TypeScript (components/website/src/lib/sdlc/brain-links.ts) — dieser Test laesst die Bash-
...
@test "T002465 Die drei belegten Slug-Paare stimmen zwischen Pipeline und TS-Kopie ueberein" {
  local ts_rule="$REPO/components/website/src/lib/sdlc/brain-links.ts"
  [ -f "$ts_rule" ] || {
    echo "fehlt: components/website/src/lib/sdlc/brain-links.ts (Task 4 noch offen)" >&2
    return 1
  }
  ...
  for pair in "${pairs[@]}"; do
    ...
    actual_ts="$(
      cd "$REPO/components/website" && npx tsx -e "import {slugForSource} from './src/lib/sdlc/brain-links.ts'; process.stdout.write(slugForSource('$rel'))" 2>/dev/null
    )"
...
@test "T002465 Ein weggeprunter Pfad unter components/website/ ist nicht ingestiert (Grenze)" {
  local ts_rule="$REPO/components/website/src/lib/sdlc/brain-links.ts"
  [ -f "$ts_rule" ] || {
    echo "fehlt: components/website/src/lib/sdlc/brain-links.ts (Task 4 noch offen)" >&2
    return 1
  ...
  # exit 0 = ingestiert (unerwuenscht fuer components/website/), exit 1 = nicht ingestiert.
  run bash -c "cd '$REPO/website' && npx tsx -e \"import {isIngestedSource} from './src/lib/brain-links.ts'; process.exit(isIngestedSource('components/website/src/pages/index.astro') ? 0 : 1)\""
  [ "$status" -eq 1 ]
}
