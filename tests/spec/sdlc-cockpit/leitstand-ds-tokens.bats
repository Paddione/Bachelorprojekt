...
# T8 -- Staleness-Guard (Review-Follow-up T007968): _tokens.css ist eine
# verbatim Kopie des Quell-CSS. build.mjs laeuft manuell; dieser Test faengt
# Drift, wenn sdlc-leitstand.css weiterlaeuft, ohne dass
# design/leitstand-ds/build.mjs erneut ausgefuehrt wurde.
@test "leitstand-ds: _tokens.css is a verbatim copy of sdlc-leitstand.css (staleness guard)" {
  TOKENS_FILE="$REPO_ROOT/design/leitstand-ds/_tokens.css"
  [ -f "$CSS_FILE" ] || skip "sdlc-leitstand.css not yet present"
  [ -f "$TOKENS_FILE" ] || { echo "fehlt: design/leitstand-ds/_tokens.css"; return 1; }
  # Positiv-Anker: _tokens.css traegt den GENERATED-Header (Herkunft aus
  # build.mjs, keine Handpflege).
  head -n 2 "$TOKENS_FILE" | grep -q 'GENERATED by design/leitstand-ds/build.mjs'
  # Negativ: Inhalt == Header + Quell-CSS (verbatim, kein Regex-Extrakt).
  expected="$BATS_TEST_TMPDIR/expected-tokens.css"
  ...
  diff -q "$expected" "$TOKENS_FILE" >/dev/null \
    || { echo "_tokens.css ist stale — 'node design/leitstand-ds/build.mjs' ausfuehren und committen"; return 1; }
}
