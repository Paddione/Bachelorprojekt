...
@test "T001811: auto-close-merged.sh extracts the ticket ID (regression)" {
  ...
  # matched anything for either brand. [T003797] the extraction moved into the
  # shared extract_ticket_ids_from_title() function; exercise it directly
  # (source, not grep) so a future regression here fails loudly.
  run bash -c 'source "'"$REPO"'/scripts/factory/auto-close-merged.sh"; extract_ticket_ids_from_title "fix(admin): cockpit foo [T001655]"'
  [ "$status" -eq 0 ]
  [ "$output" = "T001655" ]

  run bash -c 'source "'"$REPO"'/scripts/factory/auto-close-merged.sh"; extract_ticket_ids_from_title "chore: no ticket tag here"'
  [ "$status" -eq 0 ]
  [ -z "$output" ]
}
