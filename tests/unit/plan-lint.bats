...
@test "B1 math: baselined file uses max(limit, baseline.metric)" {
  # Test with a mock baseline to verify the max(limit, baseline.metric) logic deterministically
  local tmp_base
  tmp_base=$(mktemp)
  cat > "$tmp_base" <<'EOF'
{
...
EOF
  run env PLAN_LINT_SELFTEST=0 bash -c "export BASELINE='$tmp_base'; bash '$LINT' '$FIX/over-threshold.md'"
  rm -f "$tmp_base"
  [ "$status" -eq 0 ]
