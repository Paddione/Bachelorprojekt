...
@test "SCS-2: components/website/src/pages/sdlc/api/codesearch.ts exists" {
  [[ -f "$PROJECT_DIR/components/website/src/pages/sdlc/api/codesearch.ts" ]]
}

@test "SCS-2: codesearch API requires admin auth" {
  run grep -c 'isAdmin' "$PROJECT_DIR/components/website/src/pages/sdlc/api/codesearch.ts"
  [[ "$output" -ge 1 ]]
}

@test "SCS-2: codesearch API validates query parameter q" {
  run grep -c "searchParams.get('q')" "$PROJECT_DIR/components/website/src/pages/sdlc/api/codesearch.ts"
  [[ "$output" -ge 1 ]]
}

@test "SCS-2: codesearch API returns 503 when embedding service unavailable" {
  run grep -c 'embedding service unavailable' "$PROJECT_DIR/components/website/src/pages/sdlc/api/codesearch.ts"
  [[ "$output" -ge 1 ]]
}

@test "SCS-2: codesearch API supports augmented query parameter" {
  run grep -c 'augmented' "$PROJECT_DIR/components/website/src/pages/sdlc/api/codesearch.ts"
  [[ "$output" -ge 2 ]]
}

@test "SCS-2: components/website/src/lib/sdlc/codesearch-db.ts exists" {
  [[ -f "$PROJECT_DIR/components/website/src/lib/sdlc/codesearch-db.ts" ]]
}

@test "SCS-2: codesearch-db.ts has searchCode function" {
  run grep -c 'export async function searchCode' "$PROJECT_DIR/components/website/src/lib/sdlc/codesearch-db.ts"
  [[ "$output" -ge 1 ]]
}

@test "SCS-2: codesearch-db.ts uses pgvector cosine distance operator" {
  run grep -c '<=>' "$PROJECT_DIR/components/website/src/lib/sdlc/codesearch-db.ts"
  [[ "$output" -ge 1 ]]
}

@test "SCS-3: codesearch-db.ts has searchCodeAugmented function" {
  run grep -c 'export async function searchCodeAugmented' "$PROJECT_DIR/components/website/src/lib/sdlc/codesearch-db.ts"
  [[ "$output" -ge 1 ]]
}

@test "SCS-3: searchCodeAugmented queries file_dependencies for 1-hop neighbors" {
  run grep -c 'file_dependencies' "$PROJECT_DIR/components/website/src/lib/sdlc/codesearch-db.ts"
  [[ "$output" -ge 1 ]]
}

@test "SCS-3: augmented neighbors get score=0.7" {
  run grep 'score: 0.7' "$PROJECT_DIR/components/website/src/lib/sdlc/codesearch-db.ts"
  [[ "$status" -eq 0 ]]
}
