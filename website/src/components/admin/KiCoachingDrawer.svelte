<script lang="ts">
  ...
  interface KiConfig {
    ...
    isActive: boolean; modelName: string | null; displayName: string;
    apiKey: string | null; apiEndpoint: string | null;
    temperature: number | null; maxTokens: number | null; topP: number | null;
    systemPrompt: string | null; notes: string | null;
    topK: number | null; thinkingMode: boolean;
    presencePenalty: number | null; frequencyPenalty: number | null;
    safePrompt: boolean; randomSeed: number | null;
    organizationId: string | null; euEndpoint: boolean;
  ...
  let editForm = $state({
    ...
    temperature: '', maxTokens: '', topP: '', systemPrompt: '',
    topK: '', thinkingMode: false, presencePenalty: '', frequencyPenalty: '',
    safePrompt: false, randomSeed: '', organizationId: '', euEndpoint: false,
  });
  ...
  function startEdit(p: KiConfig) {
    editId = p.id;
    editForm = {
      modelName: p.modelName ?? '', displayName: p.displayName,
      apiKey: '', apiEndpoint: p.apiEndpoint ?? '',
      temperature: p.temperature?.toString() ?? '', maxTokens: p.maxTokens?.toString() ?? '',
      topP: p.topP?.toString() ?? '', systemPrompt: p.systemPrompt ?? '',
      topK: p.topK?.toString() ?? '', thinkingMode: p.thinkingMode,
      presencePenalty: p.presencePenalty?.toString() ?? '',
      frequencyPenalty: p.frequencyPenalty?.toString() ?? '',
      safePrompt: p.safePrompt, randomSeed: p.randomSeed?.toString() ?? '',
      organizationId: p.organizationId ?? '', euEndpoint: p.euEndpoint,
    };
  }
  ...
  async function saveEdit() {
    ...
    const numKeys = ['temperature', 'maxTokens', 'topP', 'topK', 'presencePenalty', 'frequencyPenalty', 'randomSeed'];
    ...
    const res = await fetch(`/api/admin/coaching/ki-config/${editId}`, {
  ...
  async function setActive(provider: string) {
    const res = await fetch('/api/admin/coaching/ki-config/active', {
      method: 'PATCH', headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ provider }),
    });
    if (!res.ok) {
      const body = await res.json().catch(() => ({}));
      showtoast(body.error ?? 'Aktivieren fehlgeschlagen');
      return;
    }
    await load();
  }
  ...
  async function createNew() {
    if (!newDisplayName.trim() && !newCatalogId) {
      showtoast('Name oder Katalog-Provider erforderlich');
      return;
    }
    const body: Record<string, unknown> = { displayName: newDisplayName.trim() };
    if (newCatalogId) body.catalogId = newCatalogId;
    else body.slug = newDisplayName.trim().toLowerCase().replace(/[^a-z0-9]/g, '-');
    ...
    if (!res.ok) {
      ...
      showtoast(errBody.error ?? 'Erstellen fehlgeschlagen');
      return;
    }
    newProvider = false;
    newCatalogId = '';
  ...
  async function doDelete(id: number) {
    const res = await fetch(`/api/admin/coaching/ki-config/${id}`, { method: 'DELETE' });
...
<aside class="drawer">
  ...
  {#if loadError}
    <div class="err-banner">&#9888; {loadError} <button onclick={load}>Erneut laden</button></div>
  {/if}
  ...
  {:else}
    <ul class="provider-list">
      {#each providers as p (p.id)}
        <li class:active={p.isActive}>
          <div class="row">
            <span class="name">{p.displayName} <span class="slug">({p.provider})</span></span>
            {#if p.isActive}
              <span class="badge active">aktiv</span>
            {:else}
              <button class="set-active" onclick={() => setActive(p.provider)}>Aktivieren</button>
            {/if}
            <button onclick={() => startEdit(p)} aria-label="bearbeiten">&#x270F;&#xFE0F;</button>
            {#if confirmingDelete === p.id}
              <button class="danger" onclick={() => doDelete(p.id)}>Wirklich löschen?</button>
            {:else}
              <button onclick={() => (confirmingDelete = p.id)} aria-label="löschen">&#x1F5D1;&#xFE0F;</button>
            {/if}
          </div>
          {#if p.modelName}
            <p class="sub">Modell: {p.modelName}{p.temperature != null ? ` · Temp: ${p.temperature}` : ''}{p.apiEndpoint ? ` · ${p.apiEndpoint}` : ''}</p>
          {/if}
    ...
    {#if newProvider}
      <div class="new-form">
        ...
        <select bind:value={newCatalogId}>
        ...
        {/if}
        <div class="actions">
          <button onclick={createNew}>Erstellen</button>
          <button onclick={() => (newProvider = false)}>Abbrechen</button>
        </div>
      </div>
    {:else}
      <button class="add" onclick={() => (newProvider = true)}>+ Provider hinzufügen</button>
    {/if}
  {/if}
</aside>

{#snippet editFields()}
  <form class="fields" onsubmit={(ev) => { ev.preventDefault(); saveEdit(); }}>
    <label>Anzeigename <input bind:value={editForm.displayName} /></label>
    <label>Modell <input bind:value={editForm.modelName} /></label>
...
<style>
  ...
  .fields label { font-size: 12px; display: flex; flex-direction: column; gap: 2px; }
  .fields input, .fields textarea, .fields select { padding: 4px 6px; }
  .fields textarea { resize: vertical; }
  .new-form { margin-top: 12px; display: grid; gap: 8px; }
  .new-form h3 { font-size: 14px; margin: 0; }
