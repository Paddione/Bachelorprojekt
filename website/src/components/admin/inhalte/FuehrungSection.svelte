<script lang="ts">
  ...
  async function save() {
    saving = true; msg = '';
    try {
      const res = await fetch('/api/admin/fuehrung/save', {
        method: 'POST',
...
<div class="pt-6 pb-20 space-y-10">
  <div class="flex justify-between items-start">
    <div>
      <h2 class="text-2xl font-bold text-light font-serif">Führung &amp; Persönlichkeit</h2>
      <p class="text-muted mt-1 text-sm">Inhalte der Seite /fuehrung-persoenlichkeit bearbeiten</p>
    </div>
  ...
  <div class={sectionCls}>
    ...
    <div>
      <label class={labelCls}>Persönliche Notiz / zweiter Absatz (wird nach der Einleitung angezeigt, Zeilenumbrüche möglich)</label>
      <textarea bind:value={data.introNote} rows={6} class="{inputCls} resize-none"></textarea>
    </div>
  </div>
  ...
  <div class={sectionCls}>
    <div class="flex justify-between items-center">
      ...
      <button onclick={addForWhom}
        class="px-3 py-1.5 bg-gold text-dark rounded-lg text-xs font-semibold hover:bg-gold/80">
        + Hinzufügen
      </button>
    </div>
    <p class="text-xs text-muted">Jeden Punkt einzeln bearbeiten, neue Punkte jederzeit ergänzen oder entfernen.</p>
    {#each data.forWhom as _, i}
      <div class="flex gap-2 items-center">
        <input type="text" bind:value={data.forWhom[i]} class="{inputCls} flex-1" />
        <button onclick={() => removeForWhom(i)}
          class="text-xs text-red-400 hover:text-red-300 flex-shrink-0">✕</button>
      </div>
    {/each}
  </div>
    ...
    {#each data.process as step, i}
      <div class="p-4 bg-dark rounded-lg border border-dark-lighter space-y-2">
        <div class="grid grid-cols-2 gap-4">
          <div>
            <label class={labelCls}>Schritt-Label</label>
            <input type="text" bind:value={step.step} class={inputCls}
              placeholder="z.B. 01 — Standortbestimmung" />
          </div>
          <div>
  ...
  <div class={sectionCls}>
    ...
    <div>
      <label class={labelCls}>CTA-Link (leer = /kontakt)</label>
