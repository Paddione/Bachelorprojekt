...
// the top-level `purgeFixturesFor`/`drainOutbox`/`purgeExpiredMagicTokens`
// imports used by the DB-gated suites above are a separate, already-bound
// module instance and are unaffected by `vi.resetModules()`.
//
// `../identity` is already mocked at module scope for the whole file (see
// top), so `keycloak.deleteUser` is a `vi.fn()` here too — we just reach for
// it per-test to script its resolved value.
// ---------------------------------------------------------------------------
