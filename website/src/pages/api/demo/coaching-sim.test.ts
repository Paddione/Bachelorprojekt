...
describe('POST /api/demo/coaching-sim', () => {
  ...
  it('coach mode fills first ki_prompt beat template (no raw placeholder leaks)', async () => {
    vi.mocked(getActiveProvider).mockResolvedValue({ provider: 'local-lmstudio', apiKey: null, apiEndpoint: 'http://localhost:1234/v1', modelName: 'hermes-3', temperature: 0.7, maxTokens: null, systemPrompt: null } as any);
    mockCreate.mockResolvedValue({ choices: [{ message: { content: 'ok' } }] });
    const res = await call({ mode: 'coach', stepNumber: 1, stepName: 'Anliegen', coachInputs: {}, previousSteps: [] });
    expect(res.status).toBe(200);
    const sent = mockCreate.mock.calls[0][0] as any;
